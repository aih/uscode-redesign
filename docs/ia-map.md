...
| `/app/versions/…` | `versions/[...identifier].astro` | The amendments to this section, and with `?view=all` every recorded version including notes-only and metadata-only changes (ADR-0075) | `us/usc/[...identifier].astro:337`, `diff/[...identifier].astro:193`, `SearchResult:70`, `ErrorPage:117`, its own view switch | the text at any listed release, a diff between any two, the classification lookup for an attributed law | breadcrumb only — the page spans every release point, so it is reading none |
