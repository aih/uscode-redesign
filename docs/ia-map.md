...
| `/app/versions/…` | `versions/[...identifier].astro` | The amendments to this section, and with `?view=all` every recorded version including notes-only and metadata-only changes (ADR-0075); with `?from=&to=`, whether it changed between two dates — the verdict, the two ends, the versions in force in between and the redline (ADR-0084) | `us/usc/[...identifier].astro:337`, `diff/[...identifier].astro:193`, `SearchResult:70`, `ErrorPage:117`, `goto.astro`'s `history` redirect, its own view switch and date form | the text at any listed release, a diff between any two, the classification lookup for an attributed law | breadcrumb only — the page spans every release point, so it is reading none |
| `/app/diff/…` | `diff/[...identifier].astro` | A reading-text redline between two release points (ADR-0026) | `Timeline:92,97`, `versions/[...identifier].astro`'s from/to picker, `CompareWith` on every section page | back to the text, `/app/versions`, the source redline, the API diff | breadcrumb only — the page is about two release points, so a bar naming one would mislead |
...
| `/app/classification` | `classification/index.astro` | The classification tables: the lookup box with the table select under it, which loads the table it names (`?scope=`, ADR-0068, ADR-0072), the session being classified now, the registry of every table, and — with `?title=`, with or without `?section=` — every row ever classified to one title or one section in any of the four orders (ADR-0071), the section view linking that section's notes in the reader (ADR-0067, ADR-0070) | `SiteHeader:209`, `SiteFooter:89`, `palette.ts:70`, `classification/[congress]/[session].astro:197`, `classification/ecct.astro:80`, `ClassificationLookup` (the unscoped no-script form's action) | one table, the ECCT, a section or a title in the reader | header, footer |
| `/app/classification/<congress>/<session>` | `classification/[congress]/[session].astro` | One classification table, in public law or U.S. Code order and either direction (ADR-0071), filtered by law, law section, title or section, 50 rows to a numbered page — and the lookup scoped to the table, whose `?q=` is applied to it as a filter (ADR-0068, ADR-0072) | `classification/index.astro:430,464`, the lookup's own suggestions (`api/classification.py`'s `_app_path`), `ClassificationLookup` (the scoped form posts back to the page) | a section in the reader, govinfo, the OLRC statviewer, the ECCT, back to the index | header, footer, wide |
| `/app/classification/ecct` | `classification/ecct.astro` | The Editorial Classification Change Table — where a provision moved without a law moving it | `classification/index.astro:481`, `classification/[congress]/[session].astro:352` | back to the index; no cell links into the reader, by rule | header, footer, wide |
| `/app/goto` | `goto.astro` | The one search box's target: routes a citation to its provision, `history <citation>` to `/app/versions` with an optional date range (ADR-0084), anything else to `/app/search` | `SiteSearch:57` (form action), `search.astro:102,158`, `search/syntax.astro:91,288`, its own examples | the provision, `/app/versions`, or `/app/search` | header, footer, prefilled box |
| `/app/search` | `search.astro` | Keyword results (ADR-0028), strict by default (ADR-0031), 20 to a numbered page (ADR-0071) | `goto.astro:45,58,123`, `search/syntax.astro` examples, its own pager | a section per result, `/app/search/syntax`, `/app/goto` | header, footer, prefilled box |
...
| `/app/data/version-changes` | `data/version-changes.astro` | How a transition between two versions of a section is classified and attributed, and what the whole corpus contains, in four sortable orders (ADR-0074, ADR-0076) | `SiteFooter:132`, `about.astro:90` | `/app/classification`, one version history, the repository — and nothing else: the page reaches no data and pins no release point | header, footer |
| `/app/about` | `about.astro` | What this site is, and what it is not | `SiteHeader:266`, `SiteFooter:141,154` | `/app/releases`, `/app/docs`, `/app/search/syntax`, `/app/data/version-changes`, OLRC, the repository, the Hugging Face dataset | header, footer |
