...
(session 62, BUILDLOG). All four phases are implemented and merged: V1 (session 63, ADR-0074,
PR #61), V2 and V4 (session 65, PRs #62 and #63), V3 (session 66, ADR-0075, PR #64), hardening
(PR #65), close-out (session 90). Implementing sessions update this line and the
...
- A section renumbered away (gotcha 3) ends its identifier's timeline; the transfer is a `tr to`
  ...
  table, ADR-0065).
- *(ADR-0077 consults them; see the V5 row.)* ECCT rows (21, editorial moves) were not consulted; a moved provision's text change stayed
...
| V3 reader | implemented | `c5-version-changes-v3` | ADR-0075. `/app/versions` is two views of one document — every entry in the DOM with `data-change-kind`, `data-view` from `?view=all`, CSS filtering, the switch two links in ADR-0071's vocabulary; `frontend/src/lib/versions.ts` (Vitest) computes both runs, both diff links and the counts per entry. `previousChangedRelease` walks back past the notes-only and metadata-only groups and falls back to its old answer without change rows. Law chips are `Pub. L. 119–102` (EN DASH). **Departure from the spec:** the chip links to the lookup (`?q=`), not to the session table — the session a law falls in is not derivable from (congress, number), and resolving it would be one suggest call per chip (ADR-0075 decision 4). `/app/versions` JS is 18,250 bytes against 18,500, unchanged. **After review (PR #64):** the page reads the title's release list through ADR-0045's cache for release-point order — the folded run is sorted rather than concatenated, and an entry whose window runs backwards offers no redline (39,645 of 423,800 transitions did; the `concurrent` flag alone would have suppressed 37,951 correct ones). The From/To picker covers every recorded version in both views (scoping it to the view took it off 41,707 of 61,252 multi-version sections). Chips carry a lead-in, since ADR-0074 attributes 7,186 `notes` and 81 `structure` transitions too; the view switch is `SortBar` with `reversible={false}`. |
| V4 backfill + deploy wiring | done | `c5-version-changes-v4` / PR #63 | Full corpus back-filled locally: 489,738 change rows over 423,800 transitions (the spec's own estimate exactly), 30,250 law rows — **7m50s wall**, not the ~2 h estimated, mostly V1's post-review streaming path. Corpus-wide shares **75.1% structure / 17.1% notes / 7.8% text, text 49.2% classified** — every share within 2.5 points of the sampled table above, so the sample held. **Surprise:** `concurrent` is **77,596** corpus-wide (18.3% of transitions) — V1's Title 16 finding (recurring content, not ADR-0021 duplicates) scaled with the corpus rather than staying near the predicted ~160. `update-corpus.sh` runs `version-changes` after a load that loaded something and `--reattribute` after a classification change. **The box is back-filled too** (2026-09-01, session 91): 489,738 change rows and 30,250 law rows in **1,295s**, and every leaf of its own `--report` matches the committed development artifact but `generated_at` — two independently loaded corpora agreeing on the annotations. `docs/deploy-status.md` has the command and the re-check. |
| V5 attribution review | implemented, recompute owed | `claude/redis-caching-version-analysis-pwa-9n0ejz` | ADR-0077. A `notes` transition is `classified` by a note row; the Editorial Classification Change Table is consulted and produces `editorial`, a third attribution value, with `in_ecct`/`ecct_move` on the law row (migration `c7e2a9f4b1d0`); `build_report` carries `attribution_by_kind`, `by_release` and `coverage`; the reader page is rewritten around three questions and renders the new tables when the artifact carries them. **Owed:** `version-changes --recompute --report` over the full corpus and `make sync-verification`, then the same on the box — until then `frontend/src/data/version-changes.json` predates the report's new keys. |
| close-out | done | `c5-version-changes-closeout` | Session 90. The artifact regenerated against PR #65's widened `concurrent` (`--recompute --report`, 112s): 350 of its 352 leaf values unchanged, `concurrent` 39,645 → **77,596** and `generated_at` the two that moved. `update-corpus.sh` runs `version-changes` on a `--force` sweep as well as after a load, and reattributes only when a classification load reports a document whose `<PRE>` text actually differs. |
