...
>
> **Progress (end of Day 2's build work):** Sessions 1–6 ✅ including prior art (`docs/prior-art.md`) and the resumable backfill tool (ADR-0012, BUILDLOG 010; live-trialled, 193 tests). **Now: (1) RUN the backfill to completion — on the disposable EC2 box (docs/remote-ops.md, ADR-0013); measured ~9 GB / 40–50 h; (2) Session 7 (frontend) in parallel; (3) Session 8 (bulk load) once zips land.** Session 7's two parts: **A** — separation per ADR-0010 (reader → `/app/us/usc/…`, API → `/api/v1/…`, bare citation URL a 307 redirector); **B** — Astro 5 + TS + USWDS reader per ADR-0011, with the BUILDLOG 008 spec as the parity bar for retiring Jinja. Session 8 loads the fetched corpus in inventory `seq` order, driven by the ledger, with per-title count verification — that's when `make verify` stops being a stub. Still open: `Uslm2Parser` table/indent parity (Day 7); Day 4 polish (keyboard nav, toggles, timeline, diffs) on the Astro layout.
