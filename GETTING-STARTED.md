...
**Session 9 — Corpus completion + deep verification (Sonnet; after the backfill finishes):**
...
**Session 10 — Day 4 reader polish (Sonnet, frontend; independent of Session 9):**
...
**Session 11 — Day 5 auth + watchlist (Sonnet):**
...
**Session 12 — Day 6 performance + deploy (Opus plan first, then Sonnet):**
> Performance: (a) a (section, release) response is immutable — set ETag (already the content hash) + long Cache-Control on /api/v1 and /app section pages, with the Caddy config caching accordingly; (b) fix the recorded debt that a section page's breadcrumbs cost an extra get_toc call; (c) quick load test (hey or locust) against the top routes, numbers in the BUILDLOG. CI: add a GitHub Actions workflow running make test + make test-web on every push (Postgres service container, dev-data fixture load) — this closes the standing gap that no independent machine re-runs the suites. Deploy: plan-mode first — pick between a Hetzner/DO VPS running compose vs Fly.io given the measured database size from Session 9; then deploy db+api+frontend+Caddy behind HTTPS with the S3 mirror as the data source (mirror pull, load-all on the box). Record the choice as an ADR; smoke-test the demo URL publicly; BUILDLOG.
