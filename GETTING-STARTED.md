...
**Before Session 4 — two prerequisites the earlier plan didn't surface.** Both were found by reviewing Session 3's result against what Sessions 4–5 assume; details and rationale in PLAN.md Day 1 items 3–5.
...
**Session 3.5 — RP inventory + TOC pass (Sonnet; do this before Session 4):**
...
**Session 6 — Bulk downloader (Sonnet), start of Day 2:** *(the inventory-seeding half moves earlier, into Session 3.5 — Session 4's resolver needs real dates and ordering. What remains here is the bulk, resumable download of all titles and all ~324 RPs.)*
...
With scaffold, parser and ingest merged (Sessions 1 ✅ 2 ✅ 3 ✅), the independent tracks are now: **Session 3.5 (RP inventory + TOC pass — on the critical path to Session 4)**, **Session 5 (reader UI — build against hand-written fixture JSON now, wire to the real API later)**, and **Session 1.5 (prior-art read, which 3.5 wants first)**. Session 4 is *not* independent: it needs 3.5's structure table and release points. If you want speed over simplicity, run the reader UI alongside 3.5 with git worktrees so agents don't collide:
