...
**Session 7 — Reader interface overhaul (Sonnet, worktree; independent of Session 6 — run them in parallel):** *(spec from the Session-5 UI review, BUILDLOG 008. This lands before Day 4's polish because everything Day 4 adds sits on this layout.)*
> Overhaul the reader's interface per PLAN.md Day 2–3 "reader interface overhaul". In order:
> 1. **Mobile-first CSS**: rewrite reader.css with base styles for ~360px screens and `min-width: 40rem` / `60rem` media queries upward. Replace the fixed `[class*=indent]` margins (1.6–6.4rem) with a `--indent-step` variable: small step on phones so deep provisions keep a readable measure, 1.6rem on wide screens. Form controls ≥16px font (iOS zoom), tap targets ≥44px, `.toc .num` min-width reduced on mobile. Verify by headless screenshots at 375px and 1280px of the demo URL, the TOC, and home.
> 2. **Site navbar**: a proper `<nav>` in base.html above everything — brand, Titles (/), Release points, API docs — flat links, wraps naturally, no JavaScript, no hamburger. Breadcrumbs and the release picker move into a contextual bar below it. Add a skip-to-content link and landmark roles/aria-labels while you're in there.
> 3. **Section title**: render `§ 45f. Mineral King Valley addition authorized` as one line in the reading face — num inline before the heading, not a gray block label above it; status badge after; identifier/release meta stays below. Same fix on TOC pages.
> 4. **Navigation top and bottom**: a compact strip under the title bar (← prev · up to chapter · next →, nums only), the existing full neighbors block stays at the bottom, and a small site footer (home, title TOC, API docs, release points) on every page.
> 5. **Fix `<ref>` rendering in web/uslm_html.py + web/reader.py** — this is a live bug: source credits currently emit relative links to /us/pl/... and /us/stat/... that 404 on this site. Internal /us/usc/ refs: rewrite href to carry the page's ?release= and add title="§ num — heading" hover text via ONE batched repository lookup per page (sections can carry hundreds of refs; no N+1). Non-USC refs: map /us/pl/{congress}/{law} and /us/stat/{vol}/{page} to the govinfo link service (verify the URL patterns at govinfo.gov before hard-coding), open in the same tab, with title text saying where they lead; anything unmappable degrades to a span, never a local 404. Add tests: internal ref rewriting, pl/stat mapping, unknown-ref degradation, and a test that no rendered page contains a relative /us/pl/ or /us/stat/ href.
> 6. Keep the reader's principles: no build step, one stylesheet, the only JS is the scroll-into-view line. Run make test, screenshot before/after, update BUILDLOG.md, commit in small steps.
