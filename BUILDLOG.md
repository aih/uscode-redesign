...
- **Produced:** `scripts/icons.py`; `frontend/public/icons/` (five PNGs) + `docs/verification/icons.json`; `frontend/public/manifest.webmanifest`; `Base.astro` (manifest + apple-touch links, theme-color meta, `viewport-fit=cover`, the bootstrap's dark correction, the standalone same-tab override); `ThemeToggle.astro` (meta update in `paint()`); `site.scss` (safe-area insets on `.topbar`, `.sectionbar` in the `display: contents` band, `.usa-footer`); `search.astro` and `goto.astro` (Home › Search trails); `frontend/tests/pwa.test.ts`; `docs/js-budgets.json` (every ceiling re-measured — Base's graph grew ~600 bytes, so all 23 routes moved); `docs/adr/0079-the-reader-is-installable.md`; guide chapter 02 (ADR claimed, two-sentence "Install as an app" stub for P4 to expand); the regenerated screenshots; the spec's Status table; this entry.
- **Verified:**
