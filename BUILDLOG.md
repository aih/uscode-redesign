...
## 085 — 2026-08-31 — Session 63: Phase V1 — the transitions get their kinds and their laws

- **Tool/model:** Claude Code, Fable 5. Worktree agent on `c5-version-changes-v1`.
- **Asked:** Implement `docs/version-semantics-spec.md` Phase V1: the schema, the hashes, `ingest/version_changes.py`, the `version-changes` subcommand, the `load_release` hook, the committed 119-2 fixture slice, ADR-0074, tests; back-fill Title 16 locally. No `storage/repository.py`, `storage/postgres.py`, `api/` or `frontend/` changes.
- **Decided (ADR-0074):** as the spec drew it — migration `b6e1f0a2c9d4` adds nullable `text_hash`/`notes_hash` to `section_versions` and the `section_version_changes` + `section_version_change_laws` tables (no FK into `classification_entries`); `_build_record` computes both hashes while the element is in hand; groups order by earliest mapped release, the window is the incorporated-law-set delta honoring `excluded_laws`, classification rows match through `identifier_variants`, and the source-credit citation diff is the recorded second signal. Two calls the spec left open: a **mid-corpus `initial` group is attributed** from the title's previous completed load (the stored `window_from_release_id` stays NULL; the departure is re-derived every compute, `--reattribute` included), and the **resume skip is a count equality** (change rows = version groups) rather than "newest group has a row" — both recorded in the ADR.
- **Found:**
  - The real 119-2 table's Title 16 rows classify Pub. L. 119-102 to exactly s2201 (`nt new` + amended) and s2206 (amended) — finding 6 confirmed against the fetched file; `tests/fixtures/tbl119pl_2nd_slice.htm` is those 19 rows with the page head verbatim, wired into `make ci-data`.
  - **`concurrent` fired on 3,016 of Title 16's 34,776 transitions, none of them ADR-0021 duplicates.** The cause is recurring content — a mid-era converter serialization variant (or a revert) whose neighbouring group spans releases on both sides — which the one-row-per-group model cannot represent as a third era, so the window runs backwards (`119-102not101 → 114-139`) and the flag says so. The spec expected ~160 corpus-wide; recorded as an ADR-0074 cost.
  - Title 16's own shares: 79.5% structure / 14.2% notes / 6.3% text over 34,776 transitions, text 29.8% classified (649), 1,230 law rows — structure-heavier and less classified than the corpus-wide sample (72.7/18.9/8.4, 50.5%); the corpus-wide comparison is V4's.
- **Produced:** `db/models.py`, `alembic/versions/b6e1f0a2c9d4_add_version_change_annotations.py`, `ingest/records.py` (`text_hash_of`/`notes_hash_of`), `ingest/base.py`, `ingest/load.py` (hash writes + recompute hook), `ingest/version_changes.py`, `ingest/__main__.py` (`version-changes`), `tests/fixtures/tbl119pl_2nd_slice.htm`, `Makefile` (ci-classification-data), `tests/test_version_changes.py` (20 fast + 1 slow), `tests/test_models.py`, `docs/adr/0074-…md`, `.claude/skills/ingest-cli/SKILL.md`, spec Status table.
- **Verified:**
  - `make test` **835 passed, 2 skipped** (was 815); `make test-slow` 14 passed, 2 skipped — including the new full-sample test restating ADR-0007's measurement over the hashes (s45f identical, s2201/s2206 differ, s2201's notes too) and the migration round trip on a scratch database.
  - Against the full local corpus: s2201's `119-99 → 119-102not101` transition is `text`/`classified` with law (119, 102), `in_classification` and `in_source_credit` both true, actions `['', 'new']`; the `not`-law case attributes 115-334 to the `115-442not334 → 115-442` transition — finding 1's exact shape.
  - Title 16 back-filled: **40,073 change rows over 5,297 sections in 184 s**, `--report` sane (numbers above, artifact left uncommitted — it is corpus-partial; V4 commits the full one), a second run skips all 5,297, `--reattribute` reproduces the same 1,230 law rows.
  - Re-check with `uv run pytest tests/test_version_changes.py` and `uv run python -m ingest version-changes --title 16 --report --out /tmp`.

### 085a — review fixes (PR #61, same session)

- The fresh-context review returned nine findings; all fixed, none declined. ADR-0074 joins `INFRASTRUCTURE_ADRS` (V1 has no reader surface — the one-line frontend exception to the phase's file boundaries, and what made `make test-web` red). `concurrent` is now `>=` — equality means both groups mapped at one release, the ADR-0021 case, verified on t19 s2502 and t54 s200308; **Title 16's counts are unchanged** (3,016; it has no such duplicates). The heading comparison is whitespace-removed like every other, the deviation from the spec's "whitespace-collapsed" recorded in ADR-0074. One dash-tolerant `PL_CITATION_RE` lives in `ingest/classification.py` and `credit_laws` imports it. Release facts are read per batch, not per run, so a release point seeded mid-run cannot KeyError an hours-long backfill. `_ensure_hashes` streams `(id, xml)` through a server-side cursor, encodes and parses each fragment once (parser chosen by the parsed root's namespace via the new `parser_for_namespace`), and flushes hashes as executemany batches — **Title 5: 8,071 versions in 10 s, ~3.7x the first pass's rate**, one fragment in memory throughout. The `load_release` hook now recomputes every section whose release map gained a row (an out-of-order load moves ranges dedupe alone used to hide) plus any touched section a change row does not cover, and a hook failure warns without failing the committed load. `--report` composes with the run instead of silently dropping `--title`; `--reattribute --recompute` errors as the contradiction it is.
- Verified: `make test` 835 passed / 2 skipped; `make test-web` 404 passed (the ratchet green again); `make test-slow` 14 passed; `--recompute` over Title 16 reproduces 40,073 rows / 1,230 laws / 3,016 concurrent; titles 1, 3, 4, 5 back-filled through the streaming path.

## 086 — 2026-08-31 — Session 64: Phase V2 — the timeline answers with its annotations

- **Tool/model:** Claude Code, Fable 5. Worktree agent on `c5-version-changes-v2`, parallel with the V4 backfill agent (disjoint files per the spec's wave plan).
- **Asked:** Implement `docs/version-semantics-spec.md` Phase V2: the change annotations and laws on `SectionVersionInfo` and `PostgresRepository.versions()`, the ordering fix to the earliest mapped release, additive `VersionOut`/`VersionLawOut`. No new routes or params; no `ingest/`, `frontend/`, or `db/models.py` changes.
- **Decided:** as the spec drew it. `SectionVersionInfo` gains `change_kind`/`text_changed`/`notes_changed`/`status_changed`/`concurrent`/`attribution` (all `None` when no change row exists) and `laws: tuple[VersionLawRef, ...]`; `versions()` reads `section_version_changes` + `section_version_change_laws` in one extra query pair, assembled in Python like the existing build, laws ordered by (congress, num). Ordering is by each group's earliest mapped release, tie-broken by version id — the ADR-0066 fix, now load-bearing; `first_seen` stays in the payload and `releases[0]` is authoritative, said in the schema description. `api/schemas.py` imports `VersionLawRef` from `storage.repository` directly — `storage/__init__.py` is outside this phase's file boundary.
- **Produced:** `storage/repository.py` (`VersionLawRef`, the extended `SectionVersionInfo`, the `versions()` docstring), `storage/postgres.py` (`_version_changes`, `_change_fields`, the sort), `api/schemas.py` (`VersionLawOut`, the extended `VersionOut`), `tests/test_versions.py` (7 tests: 2 schema-shape without a database, 5 integration), spec Status table. `api/routes.py` needed nothing.
- **Verified:**
  - `make test` **842 passed, 2 skipped** (was 835). The 5 integration tests skip without `make dev-data`, like the rest of the suite.
  - The all-`None` degradation at both layers: `VersionOut.of` on a bare `SectionVersionInfo` serializes nulls and `laws: []`; deleting a section's change rows inside a never-committed session makes `versions()` answer the same.
...
## 087 — 2026-08-31 — Session 65: Phase V4 — the whole corpus gets its change rows, and the deploy keeps them

- **Tool/model:** Claude Code, Fable 5. Worktree agent on `c5-version-changes-v4`, run in parallel with V2 per the spec's wave plan.
- **Asked:** `docs/version-semantics-spec.md` Phase V4: run `python -m ingest version-changes` over the full local corpus, commit `--report`'s `docs/verification/version-changes.json`, compare against the spec's sampled table; wire `deploy/update-corpus.sh` (version-changes after a load that loaded, `--reattribute` after a classification change); record the box's owed migration + backfill in `docs/deploy-status.md`; update CLAUDE.md and the spec's Status table. No `storage/`, `api/`, or `frontend/` changes.
- **Ran:** the backfill as one resumable full-corpus pass — **7m50s wall** (00:54–01:02), not the spec's ~2 h estimate; V1's post-review streaming `_ensure_hashes` (~940 hashes/s here) is most of the difference. 59,321 sections computed (6,617 already complete from V1's runs), 440,932 change rows and hashes, 28,405 law rows; totals after: 489,738 change rows = every version group, 423,800 transitions — the spec's estimate exactly — 30,250 law rows. `--report` then confirmed completeness in 2.8 s (0 computed, 65,938 skipped).
- **Found:**
  - **The sample held.** Corpus-wide shares 75.1% structure / 17.1% notes / 7.8% text, text 49.2% classified — against the 600-section sample's 72.7 / 18.9 / 8.4 and 50.5%, every share within 2.5 points.
  - **`concurrent` is 39,645 corpus-wide** (9.4% of transitions) — V1's Title 16 finding (recurring content: converter flip-flops and reverts, not ADR-0021 duplicates) scaled with the corpus instead of staying near the ~160 the spec predicted; recorded in the spec's Status table beside V1's.
  - Title 42 dominates as expected: 127,274 of the 423,800 transitions, 86.8% of them structure-only — the 2013–2015 converter era at its heaviest.
- **Produced:** `docs/verification/version-changes.json`; `deploy/update-corpus.sh` (post-load `version-changes --quiet` gated on `LOADED > 0`, `--reattribute --quiet` after any classification load — poll-triggered or forced sweep — both warning and carrying a non-zero exit rather than aborting the chain); `docs/deploy-status.md` (the owed migration `b6e1f0a2c9d4` + one-time backfill on the box, with the SSM command and the do-not-deploy-during caveat); CLAUDE.md's version-semantics status paragraph; the spec's Status line and V4 row; this entry.
- **Verified:** `make test` 835 passed / 2 skipped (run beside the backfill); `bash -n` on the edited script; `--report` re-run is a 2.8 s complete-check. Re-check any time with `uv run python -m ingest version-changes --report`.

## 088 — 2026-08-31 — Session 66: Phase V3 — the version history defaults to the amendments

- **Tool/model:** Claude Code, Opus 5. Worktree agent on `c5-version-changes-v3`, wave 3 of the spec's plan (V2 merged first, so the annotations were on the wire).
- **Asked:** `docs/version-semantics-spec.md` Phase V3: the two-view `/app/versions`, `previousChangedRelease` → previous-*text*-changed, the section page's "amended N times" line, `Timeline` specimens on `/app/design` covering all three kinds and both views, guide chapter 04 rewritten, ADR-0075, a `?view=all` a11y route. No Python.
- **Decided:**
  - **Two views, one document** (ADR-0075). Every entry is in the DOM with `data-change-kind`; the list root carries `data-view` from `?view=all` and the stylesheet hides the `notes` and `structure` entries in the default view. One cacheable response per URL (ADR-0018), no script, and the deferred per-account default can later flip `data-view` from the pre-paint bootstrap without a markup change.
  - **A shown entry's "Unchanged through" run extends through the entries the view hides.** `frontend/src/lib/versions.ts` computes both runs per entry and CSS picks one per view; showing only the visible run would have claimed the provision left the Code between two amendments. The diff link needs no such pair — the hidden entries fold into the run above them, so the release point before a shown entry is the same label in either view.
  - **`previousChangedRelease` walks back past the notes-only and metadata-only groups** and compares against the release point immediately before the last transition that changed the words — amending ADR-0066's default, which took the group before and therefore usually offered the same words with different markup. Without change rows it answers what it answered before.
  - **Departure from the spec:** a law chip links to the classification *lookup* (`?q=Pub. L. 119-102`), not to the session table filtered to that law. The session a law falls in is not derivable from (congress, number) — the tables split by date — and resolving it would be one `/api/v1/classifications/suggest` call per chip. Recorded as ADR-0075 decision 4 and as a named cost: the chip is two clicks from its row.
  - ADR-0074 left `guide.test.ts`'s infrastructure list, where it sat marked "ingest-only until V3's reader"; chapter 04 now covers it and ADR-0075.
- **Produced:** `frontend/src/lib/versions.ts` and `frontend/tests/versions.test.ts` (23 cases); `Timeline.astro` rewritten; `versions/[...identifier].astro` (lede, sortbar view switch, `releases[0]` as the displayed start, the From/To picker offering only the entries on screen); `lib/compare.ts` + `CompareWith.astro` (hint and the stale `content_first_seen` docstring); `versionsHref(identifier, view?)`; the section page's `Version history — amended N times over M release points`; `design.astro`'s five-entry specimen covering all four change kinds, both attribution states and two law chips, rendered in both views; `docs/adr/0075-*.md`; guide chapter 04; `docs/a11y/routes.json` (+1 route), `docs/ia-map.md`, the spec's Status table; this entry. The review round added `ReleaseOrder`/`releaseOrder` and the `withheld` flag to `lib/versions.ts`, `lawsLabel`, `heading`/`reversible` on `SortBar.astro`, and the release fetch on `/app/versions`.
- **Verified:**
  - `make test-web` — **437 passed** across 25 files (was 404 in 24). `tests/versions.test.ts` is 29 of the 33 new cases, the rest `compare.test.ts` and `url.test.ts`. The guide ratchet is part of it and went red until chapter 04 claimed ADR-0075 and ADR-0074.
  - `make test-e2e` — **646 passed, 2 skipped, 0 failed** (2.8 min), against the full local corpus behind `make dev-all`. Was 630; the additions are 8 `design.spec.ts` cases, 1 `compare.spec.ts` case and 7 a11y scans.
  - `make test-a11y` — **330 passed, 329 scans**, 8 violation/route pairs over 2,910 nodes, all of them `docs/a11y/known-violations.json`'s. `?view=all` adds 7 scans and **no violation**; the 7 extra nodes are `color-contrast` on the vendored ReDoc bundle, which counts differently between runs of identical code.
  - **JS budget unchanged:** `/app/versions` measures 18,250 inline bytes against its 18,500 ceiling before and after (`docs/verification/js-bytes.json`, written by `make test-web`).
- **Reviewed:** a fresh-context review of PR #64 raised seven findings. All seven held against the code and were fixed in the same branch. One piece of the reasoning did not: finding 1 argued that `seq(from) >= seq(start)` **whenever** `concurrent` is set, which is true of ADR-0074's original clause and false once PR #65 adds the second — 37,951 of the widened flag's 77,596 transitions run forwards. The defect held; the flag was the wrong guard for it.
  - **A backwards diff link on every inverted window** (high, held). `row.from` is the previous group's *last* release and `row.start` this group's *first*; ADR-0074's first `concurrent` clause is exactly `prev.last_seq >= group.first_seq`, and `pages/diff/[...identifier].astro` does not order-normalise `?from=`/`?to=`. Measured: **39,645 of 423,800 transitions rendered a backwards or self comparison**, 58 of them a release point against itself. `/app/versions` now reads the title's release list through ADR-0045's per-title cache and `timelineRows` takes a `seq` lookup; an entry whose window does not run forwards says so and offers no redline. The `concurrent` flag alone was rejected as the guard — PR #65 widens it to **77,596**, of which **37,951 are correctly ordered** and would have lost a working comparison.
  - **The folded run was concatenated, not merged** (medium, held). Groups interleave wherever content recurs, so a shown entry's "Unchanged through" ran backwards in the middle. Sorted by `seq` now; labels cannot be sorted (gotcha 4), which is the other half of why the release list is fetched.
  - **The `concurrent` note was wrong and cited the wrong ADR** (medium, held). It blamed ADR-0021's same-release duplicates, which ADR-0074's own cost section says it is not. It now says another stored version is mapped inside the release points this one arrives across, and the withheld comparison has its own line.
  - **The From/To picker vanished from most sections** (medium, held). Scoping its options to the view took it off **41,707 of the 61,252 sections with more than one version**, and `/app/diff` is reachable from nowhere else on the page. It lists every recorded version in both views and says so when the view shows fewer.
  - **Chips on non-text entries were unexplained** (low, held). ADR-0074 attributes every transition: **7,186 `notes` and 81 `structure` transitions carry a law**. The chip list is led by **Amended by** on a `text` entry and **Public laws recorded for this change** on any other, rather than dropping 7,267 real attributions.
  - **ADR-0053's `Neighbors` defect, same shape** (low, held). No text node survived between the citation and its action word, so the accessible name was `Pub. L. 119–21new`. The space is written into the expression, and `design.spec.ts` now reads the whole string instead of `toContainText`.
  - **The view switch hand-wrote the sortbar markup** (low, held). `SortBar` gains `heading` and `reversible`; `reversible={false}` drops the arrow and the direction words and makes the option in force an inert `<span>`. `/app/design` renders the specimen.
  - Verified as sound by the reviewer and not re-checked: the group ordering match with `storage/postgres.py`, `previousChangedRelease`'s termination cases, `isAnnotated`'s all-or-nothing rule, the API field names, and `versionsHref`'s backward compatibility.
  - Rendered against the full local corpus: § 2201's history is 7 entries — 1 `initial`, 3 `structure`, 1 `notes`, 2 `text` — the default view lists 3 of them and names itself **Amendments (2)**, and the two amendments carry `Pub. L. 115–334` and `Pub. L. 119–102`. Re-check with `curl -s localhost:8000/app/versions/us/usc/t16/s2201 | grep -o 'data-change-kind="[a-z]*"'`.

## 089 — 2026-08-31 — Session 67: hardening Phase V1 against its own review

- **Tool/model:** Claude Code, Opus 5. Worktree agent on `c5-version-changes-hardening`, branched from main at 81cf7cb (V1, V2 and V4 all merged), run in parallel with a frontend agent (disjoint files).
- **Asked:** Act on a fresh-context review of the merged V1 diff (PR #61) — eleven findings, each a claim to verify against the code before fixing, and any the code refutes declined explicitly. `ingest/`, `tests/`, `docs/adr/0074-*.md`, the ingest-cli skill and this file only; `docs/verification/version-changes.json` belongs to another branch.
- **Decided (ADR-0074 amended):**
  - **`concurrent` gains a second clause.** A transition is unreliable not only when the departing group outlives the arrival, but when any *third* group is mapped inside the window — which is what recurring content produces. Under the first clause alone the transition *after* a recurrence was unflagged while its window ran across the era the earlier group held.
  - **The incremental hook computes only what a load can have moved.** `sections_needing_recompute` names four cases (a new version group; a version no change row covers; a group the title's previous completed load did not map this section to; a group that departs into a successor) and returns the whole title only for a load below the title's newest, where an inserted release reorders groups and moves every mid-corpus `initial` window.
  - **A failed hook deletes the rows it was recomputing**, so the failure survives the process that warned about it: the resume skip is a count equality, which sees a missing row and not a stale one.
  - **`load-all --defer-version-changes`** skips the hook for a bulk run, to be followed by one `version-changes` pass — the pass `deploy/update-corpus.sh` already runs after every load.
- **Found:**
  - **The whole-title hook was real and the fix is worth about a third.** Measured against the newest release of each title: Title 42 8,465 sections / 136,213 change rows / 27.0s → 1,590 sections / 89,520 rows / 17.4s; Title 16 5,095 → 1,124. It is not the ~2 sections the arithmetic suggests, because the recurrence rule alone claims 1,122 of Title 16's 1,124 — those sections' in-force text is an older group, so extending its range moves a successor's window. It cannot be narrowed to an `UPDATE` of that window, since a widened range can also flip a third, later transition to `concurrent`.
  - **Batching a session without expunging it is quadratic.** A commit expires every instance in the identity map, so a session holding the rows of every batch pays for all of them at each later commit: Title 42's 1,590-section set took 51.6s in eight batches, 24.7s in one transaction, and **17.4s** in eight batches each expunged after its commit.
  - **The bind-limit claim holds through `_ensure_hashes`, not through the section lists.** The largest title is 8,939 sections (~18k identifier variants), well under Postgres's 65,535 bind parameters; but a post-migration hook over Title 42 would hand `_ensure_hashes` all 136,213 of its version groups in one `IN (...)`. Every id list in the module is now chunked at 5,000.
  - **The new clause moves exactly one number.** `--recompute` over the full local corpus (**180s**, hashes already stored): `concurrent` **39,645 → 77,596** (9.4% → 18.3% of 423,800 transitions); change rows, transitions, every `by_kind` count and share, `text_classified`, `law_rows` and every per-title breakdown are identical.
  - **Load-time and backfill hashes agree byte for byte** — now a test, not an assumption.
  - Mapless version groups (finding 8's phantom range) are **0 corpus-wide**; the invented range stays, and is now counted and reported rather than passed over.
- **Declined, with the reason:**
  - **`--report` cannot write partial numbers.** `build_report` reads the stored rows corpus-wide and takes no title filter, so `--title 16 --report` reports the whole corpus as it then stands. The cwd-relative default path was real and is fixed (`ingest/verification.py` anchors `docs/verification/` on the repository root); the typo'd `--title` exiting 0 was real and now raises `UnknownTitleError` (exit 2).
  - **`compute_for_sections`'s `releases` parameter was used, not dead** — but no caller passed it, so it is gone and the function reads the facts itself, keeping V1's reason for re-reading them per batch.
  - **The migration round trip stays in the default run.** Measured at 2.4s, and CI runs `make test` alone, so `@pytest.mark.slow` would take the suite's only migration round trip out of every push. The fixed scratch-database name it shared the finding with was a real race — two worktree agents against one local Postgres — and is now named for the process.
- **Produced:** `ingest/verification.py` (new — `VERIFICATION_DIR` and one JSON writer, shared with `ingest/classification.py`), `ingest/records.py` (`squash`, the one normalizer, was written three times), `ingest/version_changes.py`, `ingest/load.py`, `ingest/load_all.py`, `ingest/__main__.py`, `tests/test_version_changes.py` (+6 tests), `docs/adr/0074-…md`, `.claude/skills/ingest-cli/SKILL.md`, this entry. Six commits on `c5-version-changes-hardening`.
- **Owed:** `docs/verification/version-changes.json` still reports `concurrent: 39,645` and is owned by another branch; regenerate it with `python -m ingest version-changes --recompute --report` once both merge, and update the number in ADR-0074, CLAUDE.md's version paragraph and the spec's V4 row. The regenerated file will also re-order its keys — the shared writer sorts them, as `classification-*.json` already did.
- **Verified:** `make test` **847 passed, 2 skipped** (was 842); `make test-slow` 15 passed, 2 skipped. Timings re-checkable against a loaded corpus by calling `version_changes.sections_needing_recompute` for a title's newest release and timing `compute_in_batches` over the result; the corpus numbers with `python -m ingest version-changes --recompute --report --out <somewhere disposable>`.

### 089a — review fixes (PR #65, same session)

- The fresh-context review of the PR returned three findings; **all three held and all three are fixed.**
  - **The deferral had no way of recording itself.** `load-all --defer-version-changes` documented a plain `version-changes` follow-up, which resumes on the count equality and is blind to a group whose *range* moved — cases 3 and 4 of `sections_needing_recompute`, which change no row count. On an already-computed corpus that left `window_from_release_id`, `attribution` and `concurrent` stale on exactly the recurrence sections. A deferred load now **deletes the change rows of every section it touched** (the whole title for a load below the title's newest), which is the mechanism the failed hook already used. Measured on Title 42: **0.61s where the rows exist, 0.13s on a corpus that has none** — the bulk case — against the 17.4s hook it replaces.
  - **The empty-`classification_entries` warning was on `run_compute` only**, not on the path a load takes — and `make dev-data` loads Title 16 with no tables, so every transition was stored `attribution = 'none'` in silence and then skipped as complete. `warn_if_unclassified` is now called by both, and says which run it is warning about.
  - **`_reattribute_sections` kept three unchunked `IN (...)` lists** (section ids, version ids, change ids); the two sized by version groups can pass 65,535 binds for a 200-section batch. All three chunk at `ID_CHUNK` now, so the module's claim holds without exception.
  - Also cleared the pre-existing unused import the reviewer noticed at `tests/test_version_changes.py:356`.
- **`deploy/update-corpus.sh` needs nothing** (it is outside this branch's scope, and was checked rather than assumed): its `load-all` does not pass `--defer-version-changes`, so the hook runs per load and the plain `version-changes` behind it is the repair pass for a hook *failure*, which now deletes its own rows. Adding the flag there would be a separate change, and would work with this fix in place.
- **Verified:** `make test` **849 passed, 2 skipped**; `make test-slow` 14 passed, 2 skipped. End to end on a scratch database: a load with no classification tables **warns**, `classification` + `--reattribute` then flips s2201 and s2206 from `none` to `classified` (2 law rows); a **deferred** redo of the newest release leaves **0 change rows**, and the plain follow-up pass reports `5,095 sections computed (0 already complete, skipped)` and restores all 5,097 rows with their attributions — where before it would have skipped every one of them as complete.

## 090 — 2026-08-31 — Session 68: closing out the version-semantics work

- **Tool/model:** Claude Code, Opus 5. One branch, `c5-version-changes-closeout`, from main at f831756 with all five version-semantics PRs merged (V1 #61, V2 #62, V4 #63, V3 #64, hardening #65).
- **Asked:** Close out `docs/version-semantics-spec.md`. Regenerate the verification artifact against PR #65's widened `concurrent` and confirm nothing else moved; carry the new number into the three places that quote it and the spec's V4 row, leaving ADR-0075's identically-sized number alone; correct CLAUDE.md, which still said V2 and V3 were the remaining phases; act on three review findings against `deploy/update-corpus.sh` that PR #63 merged without addressing, declining any the code refutes; and confirm the box's owed backfill command still reads correctly.
- **Decided:**
  - **The reattribution gate is "a table moved", not "a load ran"** (ADR-0074, decision 9 amended). `classification --force` ignores the content-hash gate, so on the weekly sweep every fetched document reads as loaded whether OLRC edited it or not, and `CLASSIFICATION_CHANGED` meant only that a load had run. `FileLoadResult` gains `content_changed` and the report a `changed` count; the CLI prints `N with new content`; `run_classification` in the script reads that, and an unreadable summary assumes a table moved, which is the direction that reattributes needlessly rather than the one that leaves law rows stale.
  - **A `--force` sweep runs `version-changes` whether or not anything loaded.** The gate was `LOADED -gt 0`, and on the ~51 weeks a year that load nothing the weekly sweep this script's own header sells as the repair backstop did not reach the step. Where the rows are complete it is the skip scan and no writes.
  - **The box's backfill command loses `--quiet`.** `--quiet` passes `on_event=None`, which is the per-title progress line, the every-tenth-batch elapsed line and the empty-`classification_entries` warning. A supervised run that a deploy can kill needs to say where it got to; the scheduled run in `update-corpus.sh` keeps the flag.
- **Found:**
  - **The regenerated artifact moves exactly one number, as PR #65 predicted.** `version-changes --recompute --report` over the full local corpus, **112s** (hashes already stored): 350 of the file's 352 leaf values are unchanged — every `by_kind` count and share, `text_classified`, `law_rows`, all 58 per-title breakdowns — with `concurrent` **39,645 → 77,596** and `generated_at` the two that moved. Keys re-sorted, `ingest/verification.py`'s shared writer.
  - **The two 39,645s are different measurements**, and only one was stale. ADR-0074's counts the `concurrent` flag; ADR-0075's counts transitions whose *window runs backwards* — a property of the data, measured by SQL, which the widened flag does not change. `frontend/src/lib/versions.ts:85` and the spec's V3 row quote the second and are correct as they stand.
- **Declined, with the reason:**
  - **The post-load repair pass needs no widening.** The finding was that its count equality cannot see a group whose range moved. It does not have to: `update-corpus.sh`'s `load-all` does not pass `--defer-version-changes`, so the per-load hook runs and `sections_needing_recompute` handles the range-moved cases inside it; the pass behind it exists for a hook *failure*, and a failed hook deletes the rows it was recomputing (PR #65), which the count equality does see. The residual is a hook killed between committed batches — a deploy recreating the `api` container mid-run — which leaves some sections rewritten and others stale at equal count. That is ADR-0074's named limitation, its repair is `--title N --recompute`, and `docs/deploy-status.md` now says so beside the owed backfill.
- **Produced:** `docs/verification/version-changes.json` (regenerated), `ingest/classification.py`, `ingest/__main__.py`, `deploy/update-corpus.sh`, `tests/test_classification_loader.py` (+2 tests), `docs/adr/0074-…md` (the Owed note discharged and removed; the `concurrent` cost and decision 9 rewritten), `docs/version-semantics-spec.md` (Status line, V4 row, a close-out row), `docs/deploy-status.md`, `CLAUDE.md`, `.claude/skills/ingest-cli/SKILL.md`, this entry.
- **Verified:** `make test` **851 passed, 2 skipped**; `make test-web` **437 passed**; `bash -n` and `shellcheck` on `deploy/update-corpus.sh` (one pre-existing SC2155 in `dump_to_mirror`, untouched). The artifact comparison is re-checkable by diffing the file against its parent commit's copy — two changed leaves. CLAUDE.md's stale counts refreshed while there: 851/437/648 tests, 329 a11y scans over 36 route entries, 2,910 nodes, 74 ADRs.

## 091 — 2026-09-01 — Session 69: the box's version-change backfill

- **Tool/model:** Claude Code, Opus 5. Branch `c5-version-changes-box-backfill` from main at `45a581f`, with PR #66 merged.
- **Asked:** Review the state of the text-versioning work now that PR #66 is merged, and trigger a reprocessing of the data if one is needed.
- **Found:**
  - **The only gap was the deployed box, and it was the one `docs/deploy-status.md` already named.** Migration `b6e1f0a2c9d4` was at head there — the 2026-08-31 18:11 deploy applied it — but `section_versions` held 489,738 rows with **0** non-null `text_hash` and **0** `section_version_changes`. The V2 API fields were live and every one of them was `null`: the shape deployed, the data absent. `classification_entries` (144,837) and `ecct_entries` (21) were present, so attribution had its source.
  - The local corpus needed nothing: `docs/verification/version-changes.json` was regenerated in session 68 against PR #65's widened `concurrent`.
  - PR #66's Deploy landed at 00:33 UTC and did **not** recreate the `api` container — #66 touched docs and `deploy/update-corpus.sh` only — so the backfill had a clear window between it and the 06:41 UTC daily poll. The poll ran afterwards at 06:41 and found nothing to do; no collision.
- **Decided:**
  - **`--report --out /app/data/manifests`, not the default.** `docs/deploy-status.md` gave the command without `--out`, which writes `docs/verification/version-changes.json` *inside* the container: `docker-compose.prod.yml` deliberately bind-mounts no source tree (the image is the source of truth in production), so that artifact goes with the next image recreate. `/app/data/manifests` is `${DATA_ROOT}/manifests` on the host, so the report survives — at `/var/lib/uscode/manifests/version-changes.json`.
  - Run detached under `setsid nohup` with the log on the host, so the SSM invocation could return and the run be polled from the database rather than held open for 20 minutes.
- **Produced:** `docs/deploy-status.md` (the version-change item rewritten from owed to done, with the command actually run, the artifact's location and two re-checks; the "Last updated" line and "What is left for you", the latter stale since the classification deploy landed), `docs/version-semantics-spec.md` (V4 row), `CLAUDE.md`, this entry. No code changed — the work was already merged; this session ran it.
- **Verified:**
  - **65,938 sections, 489,738 change rows, 30,250 law rows, 489,738 hashes back-filled in 1,295s** (21m35s, against 7m50s on the development machine — the t4g, as budgeted). 0 sections skipped as already complete, which is the empty-table start.
  - **The box's report matches the committed one on every leaf but `generated_at`**: `by_kind` counts and shares (65,938 initial / 72,469 notes at 17.1% / 318,438 structure at 75.14% / 32,893 text at 7.76%), `change_rows`, `concurrent` 77,596, `law_rows` 30,250, `text_classified` 16,201 at 49.25%, `transitions` 423,800, `sections_covered` 65,938. Two independently loaded corpora agreeing on the annotations, which is a stronger check than the run's own exit code.
  - The same numbers from the rows rather than the artifact: `select change_kind, attribution, count(*) from section_version_changes group by 1,2` gives text 16,201 classified / 16,692 none.
  - Live, through the proxy: `/api/v1/sections/us/usc/t16/s45f/versions` carries `change_kind` on all 11 groups where it carried `null` before; `/us/usc/t16/s777c` has eight `classified` text transitions, each naming a law both in the classification tables (`in_classification`) and newly in the source credit (`in_source_credit`); `/app/versions/us/usc/t16/s777c` renders 43 entries with `data-change-kind` (1 initial / 4 notes / 30 structure / 8 text), the `Pub. L. 113–159` chips with the EN DASH, and no "not computed" fallback.
  - The site stayed up throughout: load average peaked at 1.43, `/app/us/usc/t16/s45f` 200 in 0.54s, `/api/v1/status` 200 in 0.26s.

## 092 — 2026-09-01 — Session 70: a reader-facing page for the version-change data

- **Tool/model:** Claude Code, Opus 5. Branch `c5-version-data-page` from `c5-version-data-page-spec` at `ecc7a9f`, so the spec travels with the work.
- **Asked:** Build `docs/version-data-page-spec.md` — one reader route, `/app/data/version-changes`, explaining how this site classifies a version change and showing what the corpus contains, corpus-wide plus the full 56-title breakdown, sortable, reading a committed JSON rather than the database. Three sequential phases: D1 the data path and its ratchet, D2 the page, D3 the wiring. Budget real time for the ratchets a new reader route trips.
- **Decided:** ADR-0076, which records the route, the sync mechanism and the two declines the spec had already reached (ingest writing into `frontend/`, a runtime endpoint). Beyond the spec:
  - **`SortBar` had one vocabulary hard-coded.** `activeKey` was `sort.startsWith("code") ? "code" : "pl"`, so every value it did not recognise resolved to `pl` — invisible while the classification tables were its only caller. It now strips the `-desc` suffix, which is the same answer for `pl`/`code` and the right one for this page's four keys.
  - **A count column's forward direction is largest first.** `SortBar` sends an option that is not in force to its own forward direction, so `Change rows` opened at title 18a's 48 rows. The option's own words say which direction it is; `-desc` reverses it. The reversal is the forward list reversed rather than a second comparator (ADR-0071), which matters because 10 of the 56 titles tie some other title on at least one sortable column.
  - **The `<tfoot>` is the corpus totals under the rows that sum to them** — the reconciliation `versiondata.test.ts` asserts, made visible on the page rather than only in a test.
- **Found:**
  - **`make shots` has been failing on `/app/about` at 320 CSS px since 2026-08-14**, and this session's run is what said so. The Hugging Face paragraph (`34b088c`, 2026-08-14) carries `load_dataset("dreamproit/uscode", "current", split="train")` in a `<code>` — one unbreakable word **317px** wide against a 288px column, pushing the document **13px** sideways (WCAG 1.4.10). Nothing had re-run the target in between, and the images could not have shown it: `page.screenshot()` here is viewport-only, so the paragraph is below the fold and every committed `about-*.png` is byte-identical before and after the fix — the failure lives in `scrollWidth`, which only the assertion reads. Fixed with a `.code-wrap` utility — the opt-in form of the `overflow-wrap: anywhere` that `.apidocs__intro code` already gets for exactly this reason.
  - **The reader image builds with the copied artifact**, which is the ADR-0053 boundary confirmed rather than assumed: `docker compose build frontend` has `./frontend` as its context and the page imports `src/data/version-changes.json` from inside it.
  - `50a`'s `text_classified_share` is `0.0` and not `null` — it has 9 text changes and none attributed — so the dash and `0.00%` are different answers, and both are asserted.

- **Produced:** `frontend/src/pages/data/version-changes.astro`, `frontend/src/lib/versiondata.ts`, `frontend/src/data/version-changes.json`, `frontend/tests/versiondata.test.ts`, `versionDataHref` in `frontend/src/lib/url.ts`, `.vdata` in `site.scss`, the `sync-verification` target, `SortBar.astro`, `SiteFooter.astro`, `about.astro`, guide chapter 09 (a section, `covers.routes`, `covers.adrs`, one scenario), `docs/ia-map.md`, `docs/a11y/routes.json` (two entries), `docs/js-budgets.json`, `frontend/scripts/screenshots.mjs`, `docs/adr/0076-…md`, `CLAUDE.md`, this entry.
- **Verified:**
  - `make test` **851 passed, 2 skipped** (207s); `make test-web` **457 passed** (437 + the 19 in `versiondata.test.ts`, and the guide ratchet green on the new route and ADR); `make test-e2e` **661 passed, 2 skipped** (3.5m); `make test-a11y` **344 passed** — **343 scans, 8 violation/route pairs over 2,910 nodes**, every number but the scan count identical to the recorded baseline, so the two new route entries contributed no violation at any viewport, in either theme, or under forced colours.
  ...
  - **The page dates itself twice**, added after the first review: above the tables it says the figures describe the corpus as it stood on `generated_at` — 65,938 sections across 58 titles at 381 release points, newest `119-102not101` of 12 July 2026 — and under the provenance heading it says loading a further release point moves them and nothing on the page recomputes them. The date comes from the artifact; the four extent facts are hand-written, since the report does not carry release-point coverage, and ADR-0076 names the change to `ingest/version_changes.py` that would remove them. Re-verified after the edit: `make shots` exit 0, `make test-web` 457, and a11y + the guide scenarios 425 passed with the artifact unmoved at 343 scans / 8 pairs / 2,910 nodes.
...
- **Found, and fixed:** chapter 02 described a chrome two features out of date. Its footer section said **nine links under four headings** and listed nine — the footer has carried eleven since ADR-0067's classification tables and ADR-0076's version-change data, and `chrome.spec.ts` asserts eleven by href. The same paragraph's account of the header's **More** menu was missing Classification tables under Reference and Keyboard shortcuts under Help. Both lists now match what the components render. The count is no longer only prose: `footer-groups` asserts `.footnav a` is eleven, so the next link added to the footer turns the guide red rather than leaving it quietly wrong — which is the whole point of ADR-0038's executable chapters, and it had not been applied to this claim.
- **Produced:** `frontend/src/components/SiteFooter.astro`, `frontend/src/pages/about.astro`, `frontend/src/styles/site.scss` (`.doc-meta--byline`), `frontend/src/pages/guide/02-reading.md`, this entry. The byline's markup and token landed in `fb0eabf`, which swept the working tree mid-edit; `f4c9cba` carries the guide line and the log; the chapter-02 corrections are on `c5-version-data-page-byline`, a worktree branched from `f4c9cba` so the main checkout stayed clear of another agent working in it.
- **Verified:**
...
- **Tool/model:** Claude Code, Fable 5 — planning session, three parallel research agents (two Explore over the repo, one web brief).
- **Asked:** Plan a progressive web application — installable as a desktop or mobile app, simple and uncluttered, focused on searching, reading and navigating the legal content; the site keeps working as it is; end with a spec and a kickoff prompt for a clean implementation session.
- **Decided:** (all in `docs/pwa-spec.md`)
  - Everything ships under `/app` — `frontend/public/` surfaces at `/app/<path>`, so `sw.js` gets scope `/app/` with no `Service-Worker-Allowed` header, and Caddy, FastAPI and the CSP need no change (`worker-src 'self'` and `manifest-src`→`default-src 'self'` already cover it).
  - Manifest with `id`/`scope: "/app/"` (trailing slash), `display: standalone`, `launch_handler: navigate-existing`; icons generated from `static/favicon.svg` by `scripts/icons.py` (the `fonts.py` pattern) into `docs/verification/icons.json`; a `theme-color` meta the theme bootstrap sets and `ThemeToggle` updates, since the manifest cannot follow a runtime theme.
  - Hand-rolled ~120-line service worker — network-first navigations with navigation preload, LRU-40 recently-read cache, cache-first immutable assets, self-contained offline page — over Workbox/`@vite-pwa/astro`, matching the zero-client-bundle, budget-ratcheted frontend. No corpus-generation coupling: network-first bounds staleness to offline periods and the page states its own release point.
  - Standalone behavior: `display-mode: standalone` forces same-tab links (cross references and search results otherwise open browser tabs outside the app); `/app/search` and `/app/goto` gain breadcrumbs; `viewport-fit=cover` + safe-area insets.
  - Install surface is one hidden-by-default row in More→HELP (`beforeinstallprompt` on Chromium, guide-instructions link on iOS), documented in guide chapter 02.
  - Three phases, wave plan 1: P1 alone; 2: P2 ∥ P4 with P2 merging first (`docs/js-budgets.json` is the shared file). Three ADRs reserved (~0079–0081; 0077/0078 are claimed on open branches).
- **Produced:** `docs/pwa-spec.md` (design, three self-contained phase prompts with file boundaries, wave plan, status table); this entry. Branch `c5-pwa-spec`.
- **Verified:** Research only — repo facts carry file:line citations against `06caaf5` (serving surface, chrome inventory, ratchet behavior), external claims carry sources (Chrome install criteria without a SW; iOS 26 Home Screen behavior; iOS Home Screen exemption from 7-day storage eviction; `redirected`-response and `"any maskable"` traps). No code written; the spec's Status table is the tracker.

## 096 — 2026-09-02 — Session 74: the installable reader (PWA Phase P1)

- **Tool/model:** Claude Code, Fable 5. Worktree branch `c5-pwa-p1`, merged with `c5-pwa-spec` so the spec travels with the work.
- **Asked:** Implement `docs/pwa-spec.md` Phase P1 — the install identity and the standalone behavior adjustments, no service worker (that is P2), no install affordance (P4).
- **Decided:** ADR-0079, which records the manifest under `/app`, the generated icons, the live theme-color meta, the standalone same-tab links, the safe-area insets and the trails on `/app/search`/`/app/goto`, plus the declined alternatives (`window-controls-overlay`, manifest `screenshots`, `@vite-pwa/astro`). Beyond the spec's text, two facts the implementation forced:
  - **cairosvg ignores `textLength`/`lengthAdjust`**, so rendering `static/favicon.svg` naively clips "USC" at the tile's edge — the word's natural width is 89.19 of the 64 viewBox units. `scripts/icons.py` measures that width on a wide probe canvas (alpha bounding box via Pillow, cairosvg's own dependency) and applies the same x0.62789 horizontal squeeze `spacingAndGlyphs` performs in a browser. The glyphs still come from whatever font the machine resolves for the favicon's stack, recorded in ADR-0079 rather than pretended reproducible.
  - **The token block declares `--page` three times**, not two — light, dark, and the print block's `#ffffff` — so `pwa.test.ts` reads the light value as the sole declaration before the dark block and the dark value as the first inside it, rather than assuming a pair.
  - **The maskable safe zone is a circle, not the inner square**, also from the review: scaling the mark into the inner 80% *square* put the letter corners at radius ~0.41 of the tile, outside the 0.40-radius circle a strictly circular Android mask keeps. `maskable_scale()` now computes the fit from the measured ink — x0.77920 for this mark — and only the two maskable PNGs changed bytes.
  - **The safe-area pairing had a hole below 40em**, found in the merge review: `.topbar` is `display: contents` there, so its new padding rendered no box and nothing padded the header rows at document top — `viewport-fit=cover` alone would have put the bar under a standalone iPhone's status bar. `.usa-header` now carries the insets in that band: one rule pads both rows, and it owns the header background, so the inset paints as header rather than page. The review also had ADR-0079 record that at 40em and up a standalone window's inset grows the sticky stack while `--sticky-h` stays fixed, so a deep link can land behind the bar by the inset amount; the browser case is unaffected.
- **Produced:** `scripts/icons.py`; `frontend/public/icons/` (five PNGs) + `docs/verification/icons.json`; `frontend/public/manifest.webmanifest`; `Base.astro` (manifest + apple-touch links, theme-color meta, `viewport-fit=cover`, the bootstrap's dark correction, the standalone same-tab override); `ThemeToggle.astro` (meta update in `paint()`); `site.scss` (safe-area insets on `.topbar`, on `.sectionbar` and `.usa-header` in the `display: contents` band below 40em, and on `.usa-footer`); `search.astro` and `goto.astro` (Home › Search trails); `frontend/tests/pwa.test.ts`; `docs/js-budgets.json` (every ceiling re-measured — Base's graph grew 1,268 bytes per route, 18,250 → 19,518 on the Base-only routes, so all 23 routes moved); `docs/adr/0079-the-reader-is-installable.md`; guide chapter 02 (ADR claimed, two-sentence "Install as an app" stub for P4 to expand); the regenerated screenshots; the spec's Status table; this entry.
- **Verified:**
  - `make test-web` **467 passed** (457 + the 10 in `pwa.test.ts`; the guide ratchet green on ADR-0079).
  - The full Playwright suite against `make dev-all` rebuilt from this branch: **661 passed, 2 skipped, 0 failed** — including all of `sticky.spec.ts` (the referee for the safe-area padding: `env()` is 0 in a browser, and the sticky geometry did not move) and the accessibility scan, **343 scans, 8 violation/route pairs over 2,910 nodes**, `docs/verification/a11y.json` byte-identical to the committed baseline — so the new breadcrumbs on `/app/search` and `/app/goto` contributed no violation at any viewport, in either theme, or under forced colours.
  - `make shots` exit 0 — no horizontal overflow at 320 CSS px or 1280-at-200%. `search-*.png` show the new trail; the `demo-*`/`demo-video-*` deltas are the deep-link scroll position, which these files churn on routinely.
  - The served surface, through the proxy: `/app/manifest.webmanifest` 200 as `application/manifest+json`, `/app/icons/icon-192.png` 200, the meta and both links in every page's head, the breadcrumb on `/app/search?q=…`.
  - Not verifiable here: an actual install (Chromium's prompt needs HTTPS; iOS has no emulation) — the manual device pass stays owed to the deploy step, as the spec's cost 3 records.

## 097 — 2026-09-02 — Session 75: offline (PWA Phase P2)

- **Tool/model:** Claude Code, Fable 5. Worktree branch `c5-pwa-p2`, from `c5-pwa-spec` at the P1 merge (`3eda3ba`).
- **Asked:** Implement `docs/pwa-spec.md` Phase P2 — the service worker and the offline fallback; no install affordance (P4), no guide chapter (P4).
- **Decided:** ADR-0080, which records the strategy table (network-first navigations with navigation preload and an LRU-40 recently-read cache in `usc-pages-v1`; cache-first for `/app/_astro/`, `/app/fonts/`, `/app/uswds/`, `/app/icons/` in `usc-assets-v1`; `/app/preview/`, `/app/healthz`, non-GET, cross-origin and non-`/app` untouched), `skipWaiting` + `clients.claim`, `updateViaCache: "none"`, the self-contained offline page, and the declined corpus-generation coupling. Beyond the spec's text, three implementation calls:
  - **The offline page lives in `usc-assets-v1`, not the pages cache**, and the worker refuses to store `/app/offline` in `usc-pages-v1` — so the LRU trim can never evict the fallback and the page never appears in its own recently-read list. A failed precache fails the install (a non-`ok` response is thrown, and registration retries next load) rather than caching a 500 as the offline answer.
  - **The LRU is by fetch order, not insertion order**: `pageFirst` deletes before it puts, so a revisited page moves to the back of the key list and the trim drops the least recently *fetched* 41st entry.
  - **`offline.astro`'s `<style>` carries `is:inline`** — without it Astro extracts the block into a build-hashed stylesheet and the one-document precache stops sufficing. Verified on the rendered page: the only `href`/`src` in the document is the retry's empty `href`, which names the URL the page was served under.
- **Produced:** `frontend/public/sw.js`; `frontend/src/pages/offline.astro` (both themes inlined from the token values, a copy of the theme bootstrap, system fonts, the `usc-pages-v1` list, a retry link); the registration script in `Base.astro`; `/app/offline` in `UNDOCUMENTED_ROUTES` and `80` in `INFRASTRUCTURE_ADRS` (`guide.test.ts` — Phase P4 moves it into chapter 02's covers); `frontend/tests/e2e/pwa.spec.ts` (five tests, unregister + cache clear in teardown); `docs/js-budgets.json` (registration is 285 bytes in `Base`'s graph, 19,518 → 19,803 on the Base-only routes, every ceiling re-measured by the headroom rule, plus `/app/offline` at 2,000 over 1,600 measured); `docs/adr/0080-offline-is-network-first-with-a-recently-read-cache.md`; the spec's Status table; this entry.
- **Verified:**
  - `make test-web` **468 passed** (467 + the jsbudget row for `/app/offline`; the guide ratchet green on the new route and ADR-0080).
  - `make test-e2e` against `make dev-all` rebuilt from this branch: **666 passed, 2 skipped, 0 failed** (663 + the five in `pwa.spec.ts`), including the accessibility scan — **343 scans, 8 violation/route pairs over 2,910 nodes**, the committed baseline unmoved, so the worker registration and the new page contributed no violation.
  - The served surface, through the proxy: `/app/sw.js` 200 as `text/javascript`; `/app/offline` 200, self-contained (no stylesheet, no script `src`, no font fetch); registration present in every page's head.
  - The offline behaviour itself is `pwa.spec.ts`: with `context.setOffline(true)` a previously visited `/app/us/usc/t16/s45f` renders from `usc-pages-v1`, and an unvisited URL gets the offline page under the URL that failed, listing the cached section.
  - Not verified here: the LRU bound at 41 pages (no browser test visits 41 sections) — the trim is four lines read against the spec; and an installed-app offline pass on a device, owed with P1's to the deploy step.
- **Review fixes (same session, PR #73):** the cache writes moved off the response path — `pageFirst` returns the network response immediately and stores in `event.waitUntil`, so a quota failure costs the cache entry rather than serving a stale copy while online, and first byte no longer waits on the download + write; `assetFirst`'s `put` is fire-and-forget so a rejected write cannot fail an asset the network answered. Neither store accepts a `Cache-Control: no-store` response — `/app/login`, `/app/signup`, `/app/provisions`, `/app/settings` carry it (ADR-0018) — asserted in `pwa.spec.ts` after a later navigation's write has landed, since absence alone would race the `waitUntil`. `OFFLINE_REV` records that an edited `offline.astro` reaches existing clients only when `sw.js` byte-changes, and an asset-URL navigation consumes its preload so Chrome logs no cancelled-preload warning. Re-run after the fixes: `make test-web` 468 passed; `pwa.spec.ts` 5 passed against the rebuilt frontend container.

## 098 — 2026-09-02 — Session 76: the install surface (PWA Phase P4)

- **Tool/model:** Claude Code, Fable 5. Worktree branch `c5-pwa-p4`, from `c5-pwa-spec` after the P1 merge, in parallel with P2, rebased onto its merge (`32cc067`) before the browser suites ran.
- **Asked:** Implement `docs/pwa-spec.md` Phase P4 — the install affordance and the documentation — without touching `Base.astro`, `sw.js`, Python or `deploy/`; first pass in parallel with P2, then rebase onto its merge, re-measure the budgets with both phases' bytes and run the browser suites.
- **Decided:** ADR-0081 — one hidden-by-default row in the More menu's HELP group and no banner: revealed as a button by `beforeinstallprompt` (stash → `prompt()` on click, hidden on `appinstalled`, re-offered only when Chromium refires the event), as a link to the guide's install section on iOS/iPadOS Safari (WebKit shells with another name excluded — the guide describes Safari's share sheet), and never in a standalone window, checked by the script and by a `@media (display-mode: standalone)` rule both. The ADR records the two blind spots: CI's Chromium fires no `beforeinstallprompt`, so every scan sees the hidden state, and macOS Safari gets no row (File → Add to Dock has no signal to key on).
- **Produced:** `frontend/src/components/InstallApp.astro`; `SiteHeader.astro` (the row, after About); `site.scss` (the row's terms and the standalone media guard); guide chapter 02's "Install as an app" grown from P1's stub to the full section — Chromium/macOS/iOS instructions, the offline behaviour drafted from the spec's § Service worker and offline, and the `install-identity` scenario asserting the manifest link and apple-touch-icon on a fixture-corpus section page — with ADR-0080 and ADR-0081 claimed in `covers.adrs`; `docs/a11y/routes.json` and `docs/ia-map.md` gain `/app/offline`; `docs/adr/0081-one-install-row-and-no-banner.md`; CLAUDE.md's PWA status paragraph; the device-pass checklist as the opening item of `docs/deploy-status.md`'s Still owed; the spec's Status table; `docs/js-budgets.json` (every ceiling re-measured with both phases' bytes — 21,283 on the Base-only routes, the island's own 1,480 per route with its comments on top of P2's 19,803); post-rebase, ADR-0080 moved out of `INFRASTRUCTURE_ADRS` into chapter 02's covers and the offline prose reconciled against the merged ADR-0080 (saved *pages*, and the `no-store` account pages never saved); this entry.
- **Verified:**
  - `make test-web` **468 passed** — the guide ratchet green on ADR-0080 (now claimed by chapter 02) and ADR-0081, the `install-identity` scenario well formed, every budget row green on the combined measure.
  - The full Playwright suite against `make dev-all` rebuilt from the rebased branch: **674 passed, 2 skipped, 0 failed** — the guide scenarios (the new one included), `pwa.spec.ts`, and the accessibility scan, now **350 scans** (343 + the seven for `/app/offline`, which contributed no violation): **8 violation/route pairs over 2,910 nodes**, the committed baseline's `scans` count the only line of `docs/verification/a11y.json` that moved.
  - `npx astro build` clean, and the served surface through the proxy: the install row's markup in every page's More menu, hidden; `/app/offline` 200.
  - Not verifiable in CI, recorded in ADR-0081: the revealed states of the row (no `beforeinstallprompt` in the harness, no iOS emulation) — the manual device pass is the opening item of `docs/deploy-status.md`'s Still owed.
- **Review fixes (same session, PR #74):** the guide's settings-carry-over sentence scoped to Chrome/Edge (an app added from Safari keeps settings of its own — iOS Home Screen apps get their own storage container, macOS Add-to-Dock apps copy at creation); CLAUDE.md's test-count line brought to 468 / 676 / 351 and the a11y paragraph to 39 route entries / 350 scans; the `.installapp` scss comment now states the actual dark-theme behaviour (the guide link takes the dark link blue like its `.navdrop__item` siblings, the button keeps `--ink` like the toggles — only one form ever shows); `prompt()`'s rejection swallowed with `.catch(() => {})`; the whole `<li>` ships `hidden` (`installapp__row`, the standalone media guard moved onto it) so a screen reader's list count stays honest when nothing reveals; ADR-0081 records that the not-Safari exclusion misses OPiOS, DuckDuckGo and desktop-UA Chrome on iPad, whose own share menus also offer Add to Home Screen since iOS 16.4. Budgets re-measured (21,283 → 21,556 on the Base-only routes, every ceiling by the rule). Re-run: `make test-web` 468 passed; `pwa.spec.ts` + `chrome.spec.ts` 45 passed and `guide.spec.ts` 82 passed + 2 skipped against the rebuilt frontend container.

## 099 — 2026-09-02 — Session 77: PR #75 readied — the merge with main, and the review's six fixes

- **Tool/model:** Claude Code, Fable 5.
- **Asked:** Prepare PR #75 (the PWA umbrella branch, `c5-pwa-spec`) for merge: review the code, fix any issues, resolve the CLAUDE.md and BUILDLOG.md conflicts against main.
- **Decided:**
  - The conflicts were additive on both sides — main's Redis-cache work (ADR-0078, PR #71) against this branch's PWA work — so both accounts stand: CLAUDE.md carries the ADR-0078 paragraph beside the PWA one, BUILDLOG interleaves entry 094 before 095–098, and the test-count line is the merged tree's own measure (**867 / 473 / 676, 351 a11y tests** — main's line said 866 against its own collection of 867). The ADR count line moves to 79, numbered to 0081.
  - A `/code-review` of the PR confirmed thirteen candidates and refuted one; six were in the PR's own files and are fixed here. Four confirmed findings live in files this PR never touched — the version-data page wave's `aria-sort` inversion on count columns, `classified-desc` ranking null shares first, the duplicated sort-heading block, and guide 09's justifying clauses — and are recorded as backlog **B3** rather than folded into a conflicted PR.
- **Produced (the six fixes):**
  - `offline.astro`: the recently-read list decodes each label in its own try/catch — `decodeURIComponent` throws on an invalid percent sequence (`?q=50%` is a storable search URL), and the throw landed in the outer catch and hid the whole list; and the page carries the `theme-color` meta with the dark correction from `Base.astro`'s bootstrap, so a dark reader's standalone window no longer paints the manifest's light title bar over a dark page. `OFFLINE_REV` 1 → 2.
  - `sw.js`: `/app/fonts/`, `/app/uswds/` and `/app/icons/` move from cache-first-forever to cache-first with a background revalidation (`cache: "no-cache"`, so an unchanged file costs a 304) — their names are stable while `scripts/fonts.py`, `scripts/icons.py` and a USWDS bump regenerate bytes in place, and installed clients kept the old files indefinitely; only build-hashed `/app/_astro/` stays cache-first outright. And `usc-assets-v1` is bounded at 120 entries (`storeAsset`, oldest-stored first, the offline page exempt) — every deploy minted a new set of hashed names and nothing ever deleted the superseded ones.
  - `site.scss`: `.reader-wrap` — every page's `<main>` — carries `calc(1rem + env(safe-area-inset-left/right))`, closing the half of ADR-0079's pairing that stopped at the chrome; in landscape under `viewport-fit=cover` the column itself could reach the sensor housing. Computes to the same 1rem everywhere `env()` is 0.
  - `settings.astro`: one sentence beside the link-target radios saying the installed app opens links in the same window whichever option is chosen — the standalone check is ORed ahead of the stored preference and overrode an explicit choice silently (latent while ADR-0034 keeps the form off).
  - ADR-0079 and ADR-0080 amended to record all of it; `docs/pwa-spec.md`'s strategy table split accordingly; CLAUDE.md's PWA paragraph likewise; `/app/offline`'s budget 2,000 → 2,500 (2,258 measured, the headroom rule); `docs/backlog.md` gains B3.
- **Verified:**
  - `make test` **867 passed, 2 skipped** on the merged tree; `make test-web` **473 passed** (the auto-merged guide ratchet green on ADR-0078–0081, every budget row green on the re-measure).
...
## 101 — 2026-09-16 — Session 79: the half-loaded title (ADR-0082)

- **Tool/model:** Claude Code, Fable 5.1.
- **Asked:** The deployed site was missing much of its data — `/us/usc/t42/s1395b–2` 404, `/us/usc/t42/ch6` and `/us/usc/t42/ch16?release=119-102` empty. Review, fix, and put precautions in place.
- **Found:** Title 42 at 119-102 was a `title_versions` row with `sections_loaded` NULL and 0 `section_release_map` rows behind it. The box's daily check found 119-102 on 2026-09-10, loaded titles 7, 12 and 15, and the kernel OOM-killed the title 42 load at 4.8 GB after 2h47m (`dmesg`); the weekly sweep on 09-14 was killed again at 5.7 GB. `load-all` printed nothing (`--quiet`, SIGKILL), the script exited before the verify gate, and every serving query joined `title_versions` without the completion marker, so the reader picked the empty row as the newest release point for the title. `USCode/SourceCheckStale` 0, `SiteUp` 1, `behind_by` 0 throughout. The memory was `load_release` holding every `SectionVersion` row of the title (136,213, 3.8 GB of XML) to dedupe on two fields.
- **Decided:** ADR-0082 — the serving path honours the marker; the loader keeps ids and hashes; `Repository.corpus_health()` and two new `/api/v1/status` fields; `USCode/CorpusIncomplete` from an EXIT trap in `update-corpus.sh` and the `uscode-corpus-incomplete` alarm (missing data breaching). The half-loaded row on the box is deleted by hand — the session's auto mode refused every remote write over SSM, three times — and the command is in `docs/deploy-status.md`'s Still owed.
- **Produced:** `storage/repository.py` (`CorpusHealth`, `Repository.corpus_health`), `storage/postgres.py` (`_load_finished()` on four queries, `corpus_health`), `ingest/load.py` (`known_versions_statement`, `_KnownVersion`, `existing_sections` as ids), `api/schemas.py` + `api/routes.py` (`incomplete_loads`, `unloaded_titles`, `newest_complete_release`), `frontend/src/lib/currency.ts` (`describeUnloaded`, the warning ranked first) + `types.ts`, guide 03's currency section, `deploy/update-corpus.sh` (`publish_corpus_health`, the trap, the 137 note), `deploy/alarms.sh`, `scripts/load_memory.py` → `docs/verification/load-memory.json`, `data/manifests/119-102.json`, `tests/test_load_all.py` (+2), `frontend/tests/currency.test.ts` (+4), ADR-0082, CLAUDE.md, deploy-status.
- **Verified:**
  - `uv run python scripts/load_memory.py --title 42`: the lookup alone **5.48 GB → 0.17 GB** peak RSS (31.8x). The whole title 42 @ 119-102 load with the fixed loader on the development machine: **8,481 sections, 652 new / 7,829 deduped, 49.9 s, 303 MB peak** (`/usr/bin/time -l`).
  - `tests/test_load_all.py::test_an_unfinished_load_is_neither_served_nor_counted` plants a NULL-marker row at a release point newer than anything loaded: a section and a TOC are served from the release before, neither listing counts it, `corpus_health` names it three times, and the health returns to its baseline after cleanup. `test_the_dedupe_check_reads_ids_and_hashes_and_never_the_text` holds the statement to four columns.
  - `make test-web` 477 passed (guide ratchet green with ADR-0082 on chapter 03). `make test` **864 passed, 5 failed, 2 skipped** — the five are `test_api`'s bare-label and newest-release assertions and `test_status_reports_the_corpus_and_the_last_check`, which pin the CI fixture inventory (no real `119-102` label; newest `119-102not101`), and this session's local inventory refresh seeded the real `119-102` and `119-103`. The same five fail identically on unmodified `main` against this database (`git stash` and rerun); CI loads its inventory from `tests/fixtures/releasepoints.json` and is unaffected.
  - The alarm exists: `describe-alarms uscode-corpus-incomplete` → `INSUFFICIENT_DATA`, `TreatMissingData: breaching`, actions on `uscode-alerts`. It goes to `ALARM` on its own two days after creation unless the new script has deployed and published by then, which is the intended direction of failure.
  - The inventory fetched today lists **119-103 (2026-09-02)**, 15 titles including 42, which the box has not yet seen; deploying before its next daily run is what prevents the same kill.
- **Second ask, same session:** review every other title for staleness, and make a section absent from a recent release point fall back to its most recent text with a warning.
  - **Audit** (per title: newest release point in the inventory that changed it, against the newest the box holds it at, over `/api/v1/releases` and `data/uscreleasepoints.json`): 15 of 58 titles behind, all of them by 119-103 alone; plus title 42 at 119-102. Nothing else is stale. Recorded in `docs/deploy-status.md`.
  - **ADR-0083** — `get_section` falls back to the newest release point before the resolved one that publishes the section (`_last_release_holding`, one extra query on the miss path); `SectionResult.absent_from`; `served_note` writes the three-part sentence; `SectionOut.absent_from`; the reader's section page renders a warning alert (`[data-absent-from]`) linking the title at the newest release point and the former parent, and drops the bar's served-from line on that page. Backward only. `frontend/src/lib/types.ts`, guide 03 (prose + `section-absent-falls-back`, `data: corpus`, on `/us/usc/t14/s1`), guide 01's 404 sentence narrowed.
  - Measured: **5,152 of 65,938 sections** are absent from their title's newest loaded release point on the development corpus (title 10 1,139; 20 596; 42 474; 14 406).
  - Verified: `tests/test_api.py` +4 (a planted section at 119-99 alone — the fixture has no vanished section — asserting the fallback, the exact answer at 119-99, the 404 before it existed, and no `absent_from` on an ordinary section). `/app/us/usc/t14/s1` rendered through `astro dev` against the local API: the warning names 119-73 and 115-384not282not334 and the text is "Establishment of Coast Guard". `make test-web` 477 passed; `make test` 868 passed with the same five inventory-pinned failures as above and none new.

## 102 — 2026-09-16 — Session 80: did a section change between two dates? (ADR-0084)

- **Tool/model:** Claude Code, Fable 5.1. Branch `c5-changed-between`, from `main` after PR #78.
- **Asked:** For the legislative-researcher persona: review the API and add a query that (a) returns a section's full version history with the kinds and dates of its changes and (b) with `from`/`to` dates returns the portion of the history between them, whether there was a change, the kinds of change, the version at each date and a diff; then a search-box mode that sends a citation in any accepted form straight to the version history page.
- **Decided:** ADR-0084.
  - The existing `GET /sections/{identifier}/versions` grows `?from=`/`?to=` rather than a second route; each date resolves as `?date=` does everywhere. `window` carries each end as it resolved (with the section route's own `note` and the release point's `caveat` — gotcha 5 on a date range), `changed` from the two ends' guid-stripped hashes (never the count of arrivals: recurring content can come and go inside a window and read the same at both ends), `change_kinds`, and the guid-stripped diff. Every entry now carries `first_release`/`last_release` as slim stamps with dates; `releases` and `first_seen` stay.
  - The cut is a pure function in `storage/repository.py` (`versions_in_window`), walking the mapped release points by `seq`, so every `Repository` shares it and it stays on the version-resolution side of rule 1. `SectionVersionInfo.published` carries the release points as refs.
  - The windowed form shares the diff's rate limit through a dependency that reads the query string, so the plain, cached timeline stays unlimited. Both routes now memoise the redline on the *served-from* pair rather than the requested labels — a date-resolved pair is as cacheable as a pinned one.
  - **The reader does not call the windowed route.** It renders from one address for everyone against a budget sized for a person, so `/app/versions` cuts the window itself (`versionsInWindow`, the same walk in TypeScript, tested on the same shapes), from the plain timeline and two `?date=` section fetches, and diffs the reading texts as `/app/diff` does — under the diff's middleware bucket when `?from=` is present. `SectionOut` gained `content_hash` after the first render compared `undefined` to `undefined` and said "No change" over 15 changed lines.
  - `history <citation>` is a prefix on the one box beside `cites`, with `from <date> to <date>` / `between … and …` / `since <date>` read off the end in `lib/query.ts`; the citation still goes to `/api/v1/citation` (ADR-0023). A subsection opens its section's history; a title or chapter gets a message and a link; `history` with no citation is told what it needs rather than searched.
  - An empty `?from=`/`?to=` joins the middleware's stripped parameters, so the form's empty To yields the clean URL. `new Date().toISOString()` is UTC's calendar and named tomorrow in a US evening; the page uses the local one.
- **Produced:** `storage/repository.py` (`published`, `first_release`/`last_release`, `VersionWindow`, `versions_in_window`), `storage/postgres.py`, `storage/__init__.py`, `api/schemas.py` (`ReleaseStampOut`, `VersionAtDateOut`, `VersionWindowOut`, `SectionOut.content_hash`), `api/routes.py` (the route, `_limit_versions_window`, `_redline_ops`), `main.py` (the limit list), `tests/test_versions_window.py` (+18); `frontend/src/lib/{types,api,url,query,versions}.ts`, `components/VersionWindowForm.astro`, `components/VersionWindowResult.astro`, `components/Timeline.astro` (dates), `components/SiteSearch.astro` (hint), `pages/versions/[...identifier].astro`, `pages/goto.astro`, `pages/search/syntax.astro`, `pages/design.astro` (three specimens), `middleware.ts`, `styles/site.scss`; guide 04 (Between two dates, three scenarios, one `demo: true` at order 75), 05 (the `history` prefix, two scenarios), 08 (the parameters, the limit row, one scenario); `docs/a11y/routes.json` (`versions-window`), `frontend/scripts/screenshots.mjs`, `docs/ia-map.md`, `frontend/tests/{query,versions}.test.ts` (+17), `tests/e2e/goto.spec.ts` (+4), `tests/e2e/design.spec.ts` (scoped locators); ADR-0084; CLAUDE.md; this entry.
- **Verified:**
  - `make test` **886 passed, 5 failed, 2 skipped** — the five are the inventory-pinned assertions BUILDLOG 101 records failing identically on `main` against this database (the local inventory holds the real `119-102` and `119-103`); none new. The 18 new tests pass: the pure cut on six shapes (baseline, inside one entry, between release points, recurring content, absent at start, unannotated), and the route's answers for § 2201 (`changed`, `["text"]`, 11 non-equal ops, the caveat on the `to` end), § 45f (no change, an all-equal diff), `to` defaulting to today, the plain timeline's dates and `window: null`, the three 422s, the 404 before the first release point, a structural node, a provision path, and that the windowed form sheds at the diff's budget while the plain form does not.
  - `make test-web` **494 passed** — the guide ratchet green with ADR-0084 on chapters 04, 05 and 08; every JS budget row green (`/app/versions` ships no script).
  - Rendered through `astro dev` against the local API and screenshotted at 1280 and 375: the form, the verdict, the two ends (the `to` end carrying the served-from note for the uningested `119-102`), the versions in force, the redline; the refusals for `from=yesterday`, `to` alone, and two dates before 2013.
  - `history 16 usc 2201` → 307 to `/app/versions/us/usc/t16/s2201`; with `from 6/12/2026 to 7/12/2026` → the same with the range; `history 16 usc 2201(b)(1)` → the section; `history title 16` → the message; `history navigable` → the message; `history` alone → a search.
...
- **Second ask, same session:** show the site's currency — the newest release point loaded and its date — on the front page, in the footer and in the header, visible at every width without intruding.
  - **Decided:** ADR-0085. A dateline under both wordmarks; the footer's secondary section opens with the release point, its date, the day of the last check and a link to `/app/releases`; a panel on the front page. The measurements that set it: the bar is 52px with 44px targets (`chrome.spec.ts`), the 40–64em header is inside `--sticky-h`'s budget, and the desktop header is 73.5px. The dateline is inside the bar's home-link box (not a separate row), visually shortened below 30em, and paid for by the logo's margins from 64em. The footer uses dates rather than ages because pinned pages are `immutable`.
  - **Produced:** `frontend/src/lib/sitecurrency.ts` (`StatusMemo`, `dateline`, `footerCurrency`, `checkedOnDate`), `components/Dateline.astro`, `components/FooterCurrency.astro`, `components/SiteCurrencyPanel.astro`, `layouts/Base.astro` (`currency` prop), `components/SiteHeader.astro`, `components/SiteFooter.astro`, `components/SourceCurrency.astro` (the missing space between its headline and detail), `pages/index.astro`, `pages/design.astro` (specimens, `currency={false}`), `styles/site.scss`, guide 03 (prose + `site-currency`), `tests/sitecurrency.test.ts` (+8), `tests/e2e/currency.spec.ts` (+8), ADR-0085, CLAUDE.md.
  - **Verified:** header heights by Playwright at 320/375/700/1024/1280 before and after — **104/104/104/73.52/73.52 → 104/104/104/73.47/73.47**; both parts of the dateline inside its clip box at every one of those widths. The first version showed "release point" on desktop without wrapping, widening the logo 237 → 272px; `make shots` failed on every page at 1280-at-200% (20 screen px of sideways scroll, the Go button 10 CSS px past the edge), so those words are now visually hidden at every width and the logo keeps its width. `make shots` then exited 0 and the 56 PNGs are re-committed with the new chrome. `currency.spec.ts` asserts the text against `/api/v1/status`, the heights, the home link's accessible name, a click on the dateline landing on `/app/`, the footer line and link, the front page panel, and no currency in `/app/design`'s own chrome.
- **Third ask, same session:** search results said "unchanged since 119-83" for the stored version's first release point, which is usually a metadata-only change. Show the release point of the latest *text* change, and the later metadata change beside it when there is one.
  - **Decided:** `Repository.change_points(version_ids)` → `VersionChangePoints(text_since, changed_at, change_kind)` from `section_version_changes`, two queries per results page, ordered by `window_to_release_id`'s seq (the earliest mapped release, ADR-0066). `api/search.py` adds `text_changed_release`, `last_changed_release`, `last_change_kind` and leaves them null when the lookup fails or no change rows exist; `first_release` stays. The reader's `resultChangeNote` renders *text unchanged since T* and adds *· notes changed at L* or *· XML/metadata changed at L* when L differs; without change rows it keeps the old sentence.
  - **Produced:** `storage/repository.py`, `storage/postgres.py`, `storage/__init__.py`, `api/search.py`, `tests/test_search.py` (+3), `tests/test_versions.py` (+2, integration), `frontend/src/lib/{types,versions}.ts`, `components/SearchResult.astro`, `pages/design.astro` (specimen rows), guide 05 (prose + `search-result-text-change`), `tests/versions.test.ts` (+3).
  - **Verified:** `/api/v1/search?q=conservation` on the development corpus: `/us/usc/t16/s3831` text 115-442, latest 118-274not159 `structure` (the index's `first_release_label` for that version is 119-99); `/us/usc/t43/s390jj` text 113-21, notes 117-327not263; `/us/usc/t16/s6603` text and latest both 119-1. The rendered results page carries those sentences.
  - `make test` **891 passed, 5 failed, 2 skipped** — the five inventory-pinned failures BUILDLOG 101 records; none new. `make test-web` **509 passed**.
- **Verified, whole session:** `make test-e2e` against the rebuilt stack, before the zoom fix: 697 passed, 2 failed, 4 did not run — `theme.spec.ts:151` on `ERR_NETWORK_IO_SUSPENDED` (the machine slept) and `keyboard.spec.ts:123` (`[` landing count 2 against 3), which passed 3 of 3 repeated alone; a second full run **703 passed, 3 skipped, 0 failed** (706 collected). The accessibility scan: 357 scans, 8 violation/route pairs over 3,127 nodes, `docs/verification/a11y.json` unchanged. After the zoom fix: `currency.spec.ts`, `chrome.spec.ts` and `sticky.spec.ts` 77 passed; `make shots` exit 0.

## 104 — 2026-09-17 — Session 82: three e2e failures that only CI saw (PR #80)

- **Tool/model:** Claude Code, Opus 5.
- **Asked:** `make test-e2e` failed on PR #80 — `currency.spec.ts` at 320px and 1280px, and `sticky.spec.ts`'s phone anchor jump. Fix and update the PR.
- **Decided:**
  - The currency failures were the corpus, not the platform. The development corpus's newest release point is `119-102`; CI's is `119-102not101`, and that label wrapped out of the dateline's clip box at 320px (104 + 84px of text in 162px) and from 64em (147 + 87px in 238px). Below 22.5em "through" is now visually hidden as well (*07/12/2026 · 119-102not101*), and from 64em the line is 0.72rem; the line box is 0.9375rem at every size, so the header stays 104px and 73.47px. A label of up to 13 characters fits at every width from 320px; 26 of 384 labels are longer. ADR-0085 amended with an addendum.
  - The sticky failure was a race. The spec jumps to the last `[id]` in the section body, `#section-notes`, closed below 40em. In CI the browser scrolled to the closed disclosure near the page's foot before `hashchange` opened it, and scroll anchoring held the foot in place as the notes grew — the notes ended 1,605px above the viewport (read from the CI trace's screencast). `ApparatusDisclosure` now calls `scrollIntoView()` after opening a closed disclosure named by the fragment.
- **Produced:** `frontend/src/styles/site.scss`, `components/ApparatusDisclosure.astro`, `tests/e2e/currency.spec.ts` (substitutes `119-102not101` into the rendered line at each width, and checks "through" is hidden below 360px), `tests/e2e/sticky.spec.ts` (+1: runs the handler from the foot of the page, which makes the order certain), guide 03's prose, ADR-0085, CLAUDE.md, `docs/verification/js-bytes.json` (+59 bytes on the two routes that mount the disclosure script), 45 re-shot PNGs.
- **Verified:**
  - Both new assertions and the new sticky test failed against the unfixed container and pass against the rebuilt one.
  - `make test-web` **509 passed**. `make test-e2e` against the rebuilt stack **704 passed, 3 skipped, 0 failed**; the accessibility scan unchanged at 357 scans, 8 violation/route pairs over 3,127 nodes. `make shots` exit 0.

## 105 — 2026-09-18 — Session 83: the root disk filled, and took the way in with it (ADR-0086)

- **Tool/model:** Claude Code, Opus 5.
- **Asked:** The site is down. Investigate, fix, and prevent for the future.
- **Decided:** ADR-0086. Bound container logs in two places (the compose `x-logging` anchor and the box's `/etc/docker/daemon.json`, because the largest log on the box belonged to another project's compose file); watch `/` as well as `/var/lib/uscode` and make both disk alarms `treat-missing-data: breaching`; put the CloudWatch agent's config in the repository; mount the data volume by UUID and verify the mount; prune tagged images on deploy; grant `ssm:StartSession` on the session document in both ARN forms.
- **Diagnosis:** Caddy answered 502 on every surface while EC2's status checks read `ok`. SSM Run Command failed in `PT0.001S` with `ResponseCode 1` and no output — on this box only; the sibling box ran the same document fine. `uscode-site-down` had fired correctly at 04:26 EDT and the watchdog was still publishing `SiteUp=0` every minute, so cron was alive (it writes to the *data* volume) and its ~20 restarts were failing into a disk that could not take a write. The system log, once readable, said it outright: `systemd-journald: Failed to open system journal: No space left on device`, `cloud-init: OSError: [Errno 28]`, `Failed to start docker.service` three times.
- **What filled it:** 7.36 GB of `json-file` container logs in 12 files — `edge-caddy-1` **4.66 GB**, `uscode-redesign-proxy-1` 1.45, `api` 0.82, `frontend` 0.41 — beside `overlay2` at 7.6 GB and `/var/log/journal` at 2.0 GB. Neither the compose file nor the daemon set any limit.
- **Why nothing alarmed:** `uscode-disk-high` watched only `path=/var/lib/uscode`, and that metric had **zero datapoints for the life of the box** — the CloudWatch agent was configured by hand for memory alone, so the box looked monitored. `notBreaching` rendered fifteen months of no data as health.
- **Repair:** grew the root volume 20 → 40 GB; `growpart` could not take it, because cloud-init dies on `ENOSPC` before its resize module — a full disk cannot grow itself. Snapshot (`snap-03ae035ae51aab4cb`), detach, attach to a throwaway `t4g.micro` in `us-east-1a`, truncate the logs (100% → 54%), drop archived journals, `growpart` + `xfs_growfs` (→ 40 GB, 30 GB free), `xfs_repair -n` clean, reattach as `/dev/xvda`, start. Same instance, same EIP, same `.env`.
- **A second fault the repair exposed:** `/etc/fstab` mounted the data volume as `/dev/nvme1n1`. The detach/reattach reordered the NVMe devices, so that name became the **statutes** volume and it mounted at `/var/lib/uscode`. The uscode database started on the statutes cluster — `role "uscode" does not exist` — while the statutes database was already running on it; two postmasters on one data directory for ~30s, which `postmaster.pid` did not catch because each container has its own PID namespace. Caught within minutes from `df` (a 40 GB device under `/var/lib/uscode`, 6.1 GB used, against a 24 GB corpus). Stack stopped, fstab rewritten to the UUID, correct 118 GB volume mounted, the four empty bind-mount directories `rmdir`'d off the statutes volume.
- **Two IAM findings:** the live `uscode-deploy-policy` allowed `ssm:StartSession` on the instance but not on `SSM-SessionManagerRunShell`, and StartSession authorizes against both — so Session Manager, the one remaining way in, was denied. Granting the AWS-owned ARN alone still failed; the denial names the **account-owned** form. Separately, `admin-grant-bootstrap-policy.json` is still attached to `linkedlegislation-deploy` (it was meant to be detached after setup), which is why that identity could publish the policy version at all.
- **Produced:** `docker-compose.prod.yml` (`x-logging` on all six services), `deploy/cloudwatch-agent.json`, `deploy/install-cloudwatch-agent.sh`, `deploy/bootstrap-box.sh` (UUID fstab + mount verification + agent install), `deploy/alarms.sh` (`uscode-root-disk-high`; both disk alarms `breaching`), `deploy/deploy-on-box.sh` (`prune -af --filter until=168h`), `deploy/admin-grant.sh` (session-document grant, both ARNs; `ec2:ModifyVolume`), `docs/deploy.md`, ADR-0086.
- **Verified:**
  - `/health`, `/app/us/usc/t16/s45f` and `/api/v1/us/usc/t16/s45f/c/5` all 200; the reader renders `§ 45f Mineral King Valley addition authorized · 119-102not101`. `/api/v1/status`: `latest_release 119-103`, `behind_by 0`, `incomplete_loads []`, `unloaded_titles []`.
  - The watchdog logged `recovered after 244 failed probes`.
  - `disk_used_percent` publishes for `/` (26.7%) and `/var/lib/uscode` (35.5%) — the first disk datapoints this box has ever sent. All nine `uscode-*` alarms `OK`, the two disk ones now green on data rather than on its absence.
  - Log caps in force on all six uscode containers and on `edge-caddy-1`, read back from `docker inspect .HostConfig.LogConfig` — a `docker restart` is **not** enough, the container must be recreated.
  - statutes: `statutes.linkedlegislation.org/app/` 200, database reads (13 tables), db log free of `error|fatal|panic|corrupt`.
- **Left open:** the five `statutes-at-large-*` containers still have `{}` for a log config — that repository needs the same anchor, and the daemon default reaches them only when they are next recreated. `docs/verification/loadtest.json` is still stale. The bootstrap policy should be detached from `linkedlegislation-deploy`.

## 106 — 2026-09-20 — Session 84: five release points behind, and a scrape from 11,835 addresses (ADR-0087, ADR-0088)

- **Tool/model:** Claude Code, Opus 5.
- **Asked:** uscode.house.gov is current through 119-108 and this site says 119-103 — why, and make the updating timelier. Some checks on PR #81 failed. Then: the site is down again; find out why and put measures in place against bot traffic.
- **Decided:**
  - **ADR-0087 — the current release point comes from its own page.** The poll read `priorreleasepoints.htm` alone, where OLRC keeps the current release point as a commented-out `<li>` and uncomments it only once its successor supersedes it. `parse_inventory` strips comments, so the site could only ever learn of a release point after it had stopped being current — one behind, permanently, with every check reporting OK. `fetch_entries` now reads `download.shtml` too: the label from the per-title XML zip links, the date from the heading, the affected titles from the rows marked `usctitlechanged`. The docstring's claim that the commented-out `119-102` entry was "never published" was the same misreading, and is corrected. The daily cadence stands (ADR-0036); the lag is now a day, not a release point.
  - **ADR-0088 — a cookie gate on the reader, one budget for the axis.** See the measurement below. `deploy/Caddyfile` answers a request for `/app*` with no `usc_h` cookie with a 403 that sets it and reloads once, at the proxy, before Astro renders and calls the API four times. It is keyed on the hostname (`USC_GATE_HOST`, default `gate.invalid`) so the dev stack, Playwright and `make shots` never meet it. `/api/v1` is not gated — programmatic use is the point of the API — and carries a second kind of limit instead: `global_rate_limit` is one bucket for every caller outside the deployment, on the routes that pin a release point.
- **Diagnosis (the outage):** ~4 hours of total outage on 2026-09-19 (232 consecutive failed watchdog probes, `api=000 app=000`, recovering 14:22 UTC) and a failed probe every hour or two since; `uscode-cpu-credits-low` in alarm since 15:37 PT on the 19th, so a t4g.large throttled to its 30% baseline. One hour of the proxy log, 2026-09-20 12:22–13:22 UTC: **15,307 requests from 11,835 distinct addresses** — 1.3 each, across 3,758 /16s — every one presenting a plausible Chrome User-Agent, **12,895 (84%) carrying `?release=` or `?date=`**. ADR-0037's crawler list matches only agents that declare themselves; ADR-0029's per-caller budgets never saw a second request from one key.
- **What separated it from a reader:** **0 of 14,400** `/app` requests in that hour carried a `Cookie` header, those 14,400 pages drew **21** asset requests, and 9,726 of them sent `Accept: */*` with no `Sec-Fetch-*` headers at all. The top two /16s (47.82, 47.79) are a quarter of it; the rest is 3,756 more, which is why no address list was written.
- **Also fixed:** PR #81's red check — `frontend/tests/guide.test.ts` refused ADR-0086, which is host operations and belongs in `INFRASTRUCTURE_ADRS`.
- **Produced:** PRs #81, #82, #83 (merged). `ingest/inventory.py` (`parse_current_release_point`, `with_current_release_point`, `fetch_entries`), `ingest/__main__.py` (`--current-url` on `inventory` and `check`), `tests/fixtures/download_current_slice.htm`, `tests/test_inventory.py` (+6), `tests/test_source_check.py` (+1), `deploy/Caddyfile` (the gate), `deploy/watchdog.sh` (its probe sends the cookie, or the gate reads as an outage and recreates the containers every ten minutes), `docker-compose.prod.yml`, `.env.example`, `params.py` (`global_rate_limit`, `is_internal_caller`), `api/routes.py` (`_limit_axis`, `_limit_pinned`), `tests/test_rate_limit.py` (+3), `.github/workflows/ci.yml` (the Caddyfile validates with the gate on as well as off), guide chapters 03 and 08, ADR-0087, ADR-0088.
- **Verified:**
  - `python -m ingest inventory --no-seed` against the live source: **`newest: 119-108 (2026-09-11)`**, where the same command reported 119-103 before.
  - The gate, against a standalone Caddy with `USC_GATE_HOST=localhost`: `/app/us/usc/t16/s45f` → 403 with `Set-Cookie: usc_h=1; …; Secure; HttpOnly` and `Cache-Control: no-store`; the same request with the cookie, and `/app/sw.js`, `/api/v1/status`, `/robots.txt` → passed through. With `USC_GATE_HOST` at its default → passed through. `caddy validate` clean both ways.
  - `make test` **894 passed**; the 5 failures on this workstation are pre-existing and identical on `main` (a local database loaded past the CI fixture). `make test-web` 509. CI green on all three PRs.
  - Python's `ipaddress` counts the documentation ranges (`203.0.113.0/24` and friends) as **private**, so they are not usable stand-ins for an outside caller in a test of `is_internal_caller`.
- **Deployed** (`33d1cf8`, 2026-09-20 14:10 UTC), with `USC_GATE_HOST`, `USC_GATE_COOKIE` and `USC_GATE_TOKEN` added to the box's `.env` before the proxy was recreated:
