...
- **Decided:**
  ...
  - **A shown entry's "Unchanged through" run extends through the entries the view hides.** `frontend/src/lib/versions.ts` computes both runs and both diff links per entry and CSS picks one per view; showing only the visible run would have claimed the provision left the Code between two amendments.
...
- **Verified:**
  ...
  - Rendered against the full local corpus: § 2201's history is 7 entries — 1 `initial`, 3 `structure`, 1 `notes`, 2 `text` — the default view shows 3, and the two amendments carry `Pub. L. 115–334` and `Pub. L. 119–102`. Re-check with `curl -s localhost:8000/app/versions/us/usc/t16/s2201 | grep -o 'data-change-kind="[a-z]*"'`.
