...
- **Asked:** Implement `docs/pwa-spec.md` Phase P4 — the install affordance and the documentation — without touching `Base.astro`, `sw.js`, Python or `deploy/`; first pass with `make test-web` green, browser suites and the `docs/js-budgets.json` re-measure deferred to the rebase onto P2's merge.
...
- **Produced:** `frontend/src/components/InstallApp.astro`; `SiteHeader.astro` (the row, after About); `site.scss` (the row's terms and the standalone media guard); guide chapter 02's "Install as an app" grown from P1's stub to the full section — Chromium/macOS/iOS instructions, the offline behaviour drafted from the spec's § Service worker and offline, and the `install-identity` scenario asserting the manifest link and apple-touch-icon on a fixture-corpus section page — with ADR-0080 and ADR-0081 claimed in `covers.adrs`; `docs/a11y/routes.json` and `docs/ia-map.md` gain `/app/offline`; `docs/adr/0081-one-install-row-and-no-banner.md`; CLAUDE.md's PWA status paragraph; the device-pass checklist as the opening item of `docs/deploy-status.md`'s Still owed; the spec's Status table; `docs/js-budgets.json` (every ceiling re-measured for the island — 20,998 on the Base-only routes, +1,480 per route with its comments); this entry.
- **Verified:**
  - `make test-web` **467 passed** — the guide ratchet green on ADR-0081 and on chapter 02's claim of the in-flight ADR-0080 (the ratchet checks that every ADR file is claimed, not that every claim has a file yet).
  - `npx astro build` clean, so the new component compiles.
  - Deferred to the rebase onto P2's merge, by the phase prompt: re-measuring `docs/js-budgets.json` with both phases' bytes, moving ADR-0080 out of `INFRASTRUCTURE_ADRS` (P2 adds it there), reconciling ADR-0080's number and the offline prose against the merged ADR, and `make test-e2e` / `make test-a11y` against `make dev-all` — ports 8000/8001/4321 belong to the P2 session during the first pass.
