...
- **Produced:** `frontend/src/lib/versions.ts` and `frontend/tests/versions.test.ts` (23 cases); `Timeline.astro` rewritten; `versions/[...identifier].astro` (lede, sortbar view switch, `releases[0]` as the displayed start, the From/To picker offering only the entries on screen); `lib/compare.ts` + `CompareWith.astro` (hint and the stale `content_first_seen` docstring); `versionsHref(identifier, view?)`; the section page's `Version history — amended N times over M release points`; `design.astro`'s five-entry specimen covering all four change kinds, both attribution states and two law chips, rendered in both views; `docs/adr/0075-*.md`; guide chapter 04; `docs/a11y/routes.json` (+1 route), `docs/ia-map.md`, the spec's Status table; this entry.
- **Verified:**
  - `make test-web` — **431 passed** across 25 files (was 404 in 24; `tests/versions.test.ts` is 23 of the 27 new cases, the rest are `compare.test.ts` and `url.test.ts`). The guide ratchet is part of it and went red until chapter 04 claimed ADR-0075 and ADR-0074.
  - `make test-e2e` — **642 passed, 2 skipped, 0 failed** (4.3 min), against the full local corpus behind `make dev-all`. Was 630; the additions are 4 `design.spec.ts` cases, 1 `compare.spec.ts` case and 7 a11y scans.
  - `make test-a11y` — **329 scans**, 8 violation/route pairs over 2,910 nodes, all of them `docs/a11y/known-violations.json`'s. `?view=all` adds 7 scans and **no violation**; the 7 extra nodes are `color-contrast` on the vendored ReDoc bundle, which counts differently between runs of identical code.
