...
## 094 — 2026-09-02 — Session 72: Redis investigated, the attribution reviewed, the reader planned as a PWA

- **Tool/model:** Claude Code (remote), Fable 5.1. Branch `claude/redis-caching-version-analysis-pwa-9n0ejz` from main at `3fa913b`. The container had no corpus and no Redis: Postgres 16 was started locally and `make ci-data` loaded the CI fixture corpus; `redis-server` 7.0.15 was started for the timings.
- **Asked:** (1) investigate Redis caching with the FastAPI integration Redis documents, using the Redis FastAPI skill; (2) design an invalidation process under which stale data is never served; (3) review the version-change analysis, improve the logic, simplify the explanation, and incorporate the Editorial Classification Change Table; (4) plan converting the site into a progressive web application launchable from a phone, with a multi-agent plan and an assessment of which agents can be Opus and which need Fable.
- **Found, before any of it:**
  - **There is no Redis FastAPI skill** in the repository, in the user-level skills, in the claude.ai skill registry or in the plugin catalog. The Redis page (`redis.io/docs/latest/integrate/fastapi/`) is blocked by this environment's egress proxy, as is `uscode.house.gov`; its Markdown source was read from `github.com/redis/docs` (`content/integrate/fastapi/_index.md`), and the tutorial it used to describe from `redis-developer/fastapi-redis-tutorial`. The page now documents Redis's own `fastapi-redis-sdk`.
  - **`fastapi-cache2` 0.2.2 is unusable here**: `fastapi_cache/coder.py` imports `starlette.templating`, which raises without Jinja2 — the import `tests/test_architecture.py` forbids — and it pins `redis<5`. **`fastapi-redis-sdk` 0.8.0 imports clean** and requires `redis>=6`; read from its installed source, its `cache()` dependency writes `Cache-Control` from the entry's TTL and a `blake2b` weak ETag, which would overwrite ADR-0018's resolution-based policy and ADR-0007's content-hash ETag, and its limiter is a fixed window (`INCREX`, Lua fallback), not ADR-0029's token bucket.
- **Decided:**
  - **ADR-0077.** A `notes` transition is `classified` by a note row (7,186 `notes` transitions carried a law row corpus-wide and every one read `none`); the ECCT is consulted through `ecct_key(identifier)`, the inverse of `derive_usc_identifier`, and a row whose *prompting* law is in the window writes a law row flagged `in_ecct` with the move (`42:294t nt → 42:294u new`) and OLRC's own `ed chg` token; a transition with only that is **`editorial`**, a third attribution value. Migration `c7e2a9f4b1d0` adds the two columns; every carrier gains them with defaults. `build_report` gains `attribution_by_kind` (zero-filled), `by_release` (arrivals per release point per kind — the table that says whether 75% `structure` is spread or concentrated at converter changes), `notes_classified`, `editorial`, `ecct_law_rows` and `coverage` (the four facts ADR-0076 found the page writing by hand). Declined: renaming `structure`; a row per era (still ADR-0074's named follow-up); attributing `structure` transitions.
  - **The explanation is three questions.** `/app/data/version-changes` is rewritten: a version is one distinct XML; a transition is asked whether the statutory text changed, whether the notes did, and is otherwise `structure`, "the stored name for *only the XML differs*"; the laws that arrived are a set difference on the release points' names; each transition is one of three things by which table names one of those laws; `concurrent` is explained by XML that recurs. The new tables render only when the artifact carries the keys, so the committed artifact still renders.
  - **The Redis spec's contract** (`docs/redis-caching-spec.md`): a `corpus_state.generation` row bumped by statement-level triggers on the fifteen tables the API reads, inside the writer's transaction, read as the first statement of every request through `Repository.corpus_generation()` and prefixed onto every key — airtight under `READ COMMITTED` because the generation is read before the data, with the reasoning written for a reviewer. TTLs and eviction groups declined by name; the site's own `releasecache.ts` (ADR-0045) is the existing five-minute stale-server, and an `X-Corpus-Generation` header on every API response is what retires it. Response cache on six routes, a two-tier diff memo, one Lua token bucket for all nine limiters with fail-open to the in-process bucket. Four phases, Opus 5 for all, Fable for the review of R1's ordering.
  - **The PWA spec** (`docs/pwa-spec.md`): manifest under `/app/`, icons rendered from the SVG reproducibly, a post-build `sw.js` (not a page: a `.ts` endpoint is a route to the guide ratchet and SSR per fetch) whose three rules *are* ADR-0018's three `Cache-Control` classes; an offline banner drawn from an attribute the worker writes into the cached HTML, so it needs no script; Save-for-offline at the served-from release point, so a saved copy is pinned and can never be stale; an external `register-sw.js` so the inline budgets are untouched. Six agents: four Opus 5, the worker and its reviewer Fable 5.1, on the ground that those are the two places a wrong line is quiet.
- **Produced:** `ingest/version_changes.py`, `db/models.py`, `alembic/versions/c7e2a9f4b1d0_ecct_attribution.py`, `storage/repository.py`, `storage/postgres.py`, `api/schemas.py`, `tests/test_version_changes.py` (+6, the migration round-trip extended), `tests/test_versions.py`, `frontend/src/lib/{types,versions,versiondata}.ts`, `Timeline.astro`, `pages/data/version-changes.astro` (rewritten), `site.scss` (`.timeline__law-move`, `.vdata__glossary`, `.vdata__pair`), `frontend/tests/{versions,versiondata}.test.ts` (+9), guide chapters 04 and 09, `docs/adr/0077-attribute-from-both-tables.md`, pointers in ADR-0074 and the spec's V5 row, `docs/redis-caching-spec.md`, `docs/pwa-spec.md`, `CLAUDE.md`, this entry.
- **Verified:** `make test` **850 passed, 9 skipped** here (844 + 6 new; the seven skips beyond the development machine's two need an OpenSearch cluster); `make test-web` **466 passed** (457 + 9), the guide ratchet green on ADR-0077 and no new route; `npm run build` clean with the rewritten page. `python -m ingest version-changes --recompute --report` over the fixture corpus: 5,097 change rows, 2 law rows, and the report carries `attribution_by_kind` (`text: classified 2`), `by_release` (`119-99` 5,095 initial, `119-102not101` 2 text), `coverage` (`release_points_loaded 2`, newest `119-102not101` / `2026-07-12`). Timings for the Redis spec, in-process on the fixture corpus: `SELECT 1` 0.12 ms p50, `list_releases()` 4.75 ms, `get_section` 4.25 ms, Redis `GET` of 30 KB 0.07 ms. **Not run here:** `make test-e2e` and `make test-a11y` — no Docker stack in this container; the rewritten page and the timeline chip need a browser pass before merge, and `make shots` on `/app/data/version-changes` at 320 px for the new paired tables. **Owed:** the full-corpus `--recompute --report` and `make sync-verification` on the development machine, and `version-changes --reattribute` on the box, before the page shows the new tables.
- **Addendum, the same session — the ECCT's earlier pages.** Asked to research the ECCT further: earlier pages on uscode.house.gov hold many more rows than the 21 loaded. Every route to OLRC and every archive was closed from the container (`uscode.house.gov`, `web.archive.org`, `archive.ph`, Common Crawl, govinfo, the search caches; the environment's AWS keys are invalid, so neither the box nor the mirror could stand in; `dreamproit/loadusc-xcitedb` has no classification code). What could be established is about the loader: the 2026-08-11 walk linked two ECCT documents and its pattern would have dropped an archived copy named `ecct_118-2.htm`, `ecct118-2.html` or `ecct_118_2.html`. Built: tolerant naming, `ecct_session_from_page` (the rolling page names its own session), `classification --probe-ecct` (asks for every archived name back to the 104th, records the 404s), and `scripts/ecct_wayback.py` (harvests the Wayback Machine's captures of `ecct.html` into a `--from-file` directory and `docs/verification/ecct-history.json`), with tests against fakes; `docs/ecct-research.md` records the findings and the three-step procedure to run from a networked machine. `make test` **864 passed** here after the addition (+14); neither the probe nor the harvest has touched the network.
