...
runs axe-core over the route matrix in `docs/a11y/routes.json` — 26 route entries (one expanding to
every guide chapter on disk), three viewports, both themes, one `forced-colors: active` pass and six
interactive states, **244 scans in ~1m20s**, against `wcag2a`/`wcag2aa`/`wcag21a`/`wcag21aa`. A
...
`make test` = **486** Python tests; `make test-web` = **213** frontend tests; `make test-e2e` = **355**
Playwright tests, 245 of which are the accessibility scan (**all three are required** — reader
...
(39 ADRs). Read the entry you need rather than assuming; this file deliberately no longer restates them.
